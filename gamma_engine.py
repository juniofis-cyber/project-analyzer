...
def gamma_analysis_2d(
    ...
    for iy in range(h_ref):
        ...
        for ix in range(w_ref):
            ...
            jy_start = max(0, int(np.round((y_mm - search_radius_mm) / eval_res_interp)) + h_eval // 2)
            jy_end = min(h_eval, int(np.round((y_mm + search_radius_mm) / eval_res_interp)) + h_eval // 2 + 1)
            jx_start = max(0, int(np.round((x_mm - search_radius_mm) / eval_res_interp)) + w_eval // 2)
            jx_end = min(w_eval, int(np.round((x_mm + search_radius_mm) / eval_res_interp)) + w_eval // 2 + 1)

            # Verificar se janela é válida (não vazia)
            if jy_start >= jy_end or jx_start >= jx_end:
                # Janela vazia: usar dose no ponto central (sem busca na vizinhança)
                dose_eval_center = float(interpolator([[y_mm, x_mm]])[0])
