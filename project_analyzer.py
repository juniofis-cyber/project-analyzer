...
# ==================== ANÁLISE GAMMA ====================
    ...
    # Opção A: usar mapa existente da sessão (já gerado em "Vários filmes")
    ...
    # Opção B: gerar mapa de dose diretamente na aba Gamma
    if dose_film is None:
        st.markdown("---")
        st.subheader("📤 Gerar Mapa de Dose na Aba Gamma")
        st.info("Envie o filme irradiado, o filme 0 Gy (background) e a curva de calibração.")
        
        col_up1, col_up2 = st.columns(2)
        with col_up1:
            up_irr = st.file_uploader("Filme irradiado (TIFF/PNG)", type=['tif','tiff','png','jpg','jpeg'], key="g_irr")
        with col_up2:
            up_0gy = st.file_uploader("Filme 0 Gy (background)", type=['tif','tiff','png','jpg','jpeg'], key="g_0")
        
        # Curva de calibração
        curva_gamma = None
        col_curva_file, col_curva_sess = st.columns(2)
        with col_curva_file:
            curva_upload = st.file_uploader("Curva de calibração (.json)", type=['json'], key="g_curva")
            if curva_upload:
                curva_gamma = json.load(curva_upload)
        with col_curva_sess:
            if 'curva_calibracao' in st.session_state:
                if st.checkbox("Usar curva da sessão", value=True, key="g_usar_curva_sess"):
        ...
        if up_irr and up_0gy and curva_gamma:
            with st.spinner("Gerando mapa de dose..."):
                try:
                    ...
                    media_bordas_irr = np.mean([c_irr[0,:], c_irr[-1,:], c_irr[:,0], c_irr[:,-1]])
                    media_centro_irr = np.mean(c_irr[c_irr.shape[0]//3:2*c_irr.shape[0]//3, c_irr.shape[1]//3:2*c_irr.shape[1]//3])
            ...
            st.warning("⚠️ Envie todos os arquivos: filme irradiado + filme 0 Gy + curva de calibração")
            st.stop()
    ...
    arquivo_tps = st.file_uploader("Envie o arquivo de dose do TPS", type=['all', 'csv', 'txt', 'dcm', 'png', 'jpg', 'tif'], key="upload_tps")
    ...
    if arquivo_tps:
        with st.spinner("Lendo arquivo TPS..."):
            try:
                ...
                st.success(f"✅ TPS carregado: {dose_tps.shape[1]}×{dose_tps.shape[0]} px | Resolução: {res_tps_mm:.3f} mm/px | Max: {dose_tps.max():.2f} Gy")
            except Exception as e:
                st.error(f"❌ Erro ao ler arquivo TPS: {e}")
                st.stop()
    else:
        st.info("⬆️ Envie o arquivo de dose do TPS para continuar")
        st.stop()
    ...
    with col_manual:
        res_film_manual = st.number_input("mm/pixel", min_value=0.01, max_value=10.0, value=0.35, step=0.01, format="%.3f")
        dpi = st.number_input("Ou DPI", min_value=10, max_value=2400, value=72)
        if dpi > 0:
            res_film_manual = 25.4 / dpi

    usar_manual = st.checkbox("Usar valor manual", value=False)
    res_film_mm = res_film_manual if usar_manual else res_film_estimada
    st.success(f"📐 Resolução do filme: **{res_film_mm:.3f} mm/pixel**")
    ...
    try:
        ...
        ax_prev[1].set_title(f"TPS\n{dose_tps.shape[1]}×{dose_tps.shape[0]} | {res_tps_mm:.2f} mm/px")
        ax_prev[1].axis('off')
    ...
    # Registro
    st.markdown("---")
    st.subheader("3️⃣ Registro Automático")
    ...
    if st.button("🔧 Executar Registro Automático", type="primary"):
        with st.spinner("Alinhando..."):
            try:
                reg = auto_register_dose_maps(dose_film, dose_tps, res_film_mm, res_tps_mm, allow_rotation=False)
    ...
    interp = st.selectbox("Interpolação", [3, 5, 10], index=1, help="Menor = mais rápido, Maior = mais preciso")

    # Calcular Gamma
    st.markdown("---")
    if st.button("🚀 CALCULAR GAMMA", type="primary"):
        with st.spinner("Calculando gamma... (pode levar 1-3 min)"):
            try:
                # NÃO aplicar transformação no filme — passar registro para o gamma engine
                # O gamma engine busca no filme original usando as coordenadas de registro
                gamma_res = gamma_analysis_2d(
                    reference_dose=dose_tps,
                    evaluation_dose=dose_film,  # Filme ORIGINAL, não cortado
                    ...
                    interp_factor=int(interp),
                    max_gamma=2.0,
                    registration_tx_mm=tx,  # Passar registro para o engine
                    registration_ty_mm=ty,
                )
    ...
    if 'gamma_res' in st.session_state:
        ...
        try:
            ...
            # TPS
            im0 = ax_r[0,0].imshow(dose_tps, cmap='jet', origin='lower')
            ax_r[0,0].set_title(f"TPS (Referencia)\nMax: {dose_tps.max():.2f} Gy")
            ax_r[0,0].axis('off')
            plt.colorbar(im0, ax=ax_r[0,0], fraction=0.046)

            # Filme ORIGINAL (não cortado)
            im1 = ax_r[0,1].imshow(dose_film, cmap='jet', origin='lower')
            ax_r[0,1].set_title(f"Filme Original\n{dose_film.shape[1]}×{dose_film.shape[0]} | Max: {dose_film.max():.2f} Gy")
            ax_r[0,1].axis('off')
            ...
            # OVERLAY: Contornos do TPS sobre o filme (mostrando alinhamento)
            ax_r[1,1].imshow(dose_film, cmap='gray', origin='lower', alpha=0.7)
            # Contornos do TPS (cyan) sobrepostos ao filme
            levels_tps = [0.5 * dose_tps.max(), 0.8 * dose_tps.max(), 0.95 * dose_tps.max()]
            # Criar versão alinhada do TPS para visualização (zoom para tamanho do filme)
            from scipy.ndimage import zoom
            zoom_y = dose_film.shape[0] / dose_tps.shape[0]
            zoom_x = dose_film.shape[1] / dose_tps.shape[1]
            tps_on_film = zoom(dose_tps, (zoom_y, zoom_x), order=1, mode='nearest')
            # Aplicar translação visual (em pixels do filme)
            shift_px_x = int(tx / res_film_mm)
            shift_px_y = int(ty / res_film_mm)
            from scipy.ndimage import shift
            tps_shifted = shift(tps_on_film, (shift_px_y, shift_px_x), order=1, mode='constant', cval=0)
            ax_r[1,1].contour(tps_shifted, levels=levels_tps, colors='yellow', linewidths=2, origin='lower')
            ax_r[1,1].set_title("Overlay: Filme (cinza) + TPS alinhado (amarelo)")
            ax_r[1,1].axis('off')
            from matplotlib.lines import Line2D
            legend_elements = [Line2D([0], [0], color='yellow', lw=2, label='TPS alinhado')]
            ax_r[1,1].legend(handles=legend_elements, loc='upper right', fontsize=8)
