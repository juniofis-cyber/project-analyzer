...
# ==================== ANÁLISE GAMMA ====================
    ...
    if st.button("🚀 CALCULAR GAMMA", type="primary"):
        with st.spinner("Calculando gamma... (pode levar 1-3 min)"):
            try:
                # Alinhar filme
                dose_film_alg = apply_transform(dose_film, res_film_mm, tx, ty, 0.0, 1.0, target_shape=dose_tps.shape)
    ...
    if 'gamma_res' in st.session_state:
        ...
        try:
            ...
            im1 = ax_r[0,1].imshow(dose_film_alg, cmap='jet', origin='lower')
            ax_r[0,1].set_title(f"Filme Alinhado\nMax: {dose_film_alg.max():.2f} Gy")
            ax_r[0,1].axis('off')
            ...
            # OVERLAY: Contornos do filme sobre o TPS
            ax_r[1,1].imshow(dose_tps, cmap='jet', origin='lower', alpha=0.6)
            levels = [0.5 * dose_tps.max(), 0.8 * dose_tps.max(), 0.95 * dose_tps.max()]
            cs1 = ax_r[1,1].contour(dose_film_alg, levels=levels, colors='yellow', linewidths=2, origin='lower')
            cs2 = ax_r[1,1].contour(dose_tps, levels=levels, colors='cyan', linewidths=1.5, linestyles='--', origin='lower')
            ax_r[1,1].set_title("Overlay: TPS (fundo) + Filme (amarelo) + TPS (cyan)")
            ax_r[1,1].axis('off')
            from matplotlib.lines import Line2D
            legend_elements = [Line2D([0], [0], color='yellow', lw=2, label='Filme'),
                               Line2D([0], [0], color='cyan', lw=1.5, linestyle='--', label='TPS')]
