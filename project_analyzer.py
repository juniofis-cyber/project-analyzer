...
# ==================== ANÁLISE GAMMA (VERSÃO CORRETA) ====================
    ...
    # ========== PASSO 0: OBTER MAPA DE DOSE DO FILME ==========
    dose_film = None
    res_film_mm = None
    ...
    if 'mapa_dose_filme' in st.session_state:
        ...
        if usar_existente:
            ...
            # Tentar obter resolução da sessão
            res_film_mm = st.session_state.get('res_film_mm', 0.35)
            st.success(f"✅ Mapa de dose do filme: {dose_film.shape[1]}×{dose_film.shape[0]} px | {res_film_mm:.3f} mm/px")

    if dose_film is None:
        st.markdown("---")
        st.subheader("📤 Gerar Mapa de Dose")

        col1, col2 = st.columns(2)
        with col1:
            up_irr = st.file_uploader("Filme irradiado", type=['tif','tiff','png','jpg'], key="g_irr")
        with col2:
            up_0gy = st.file_uploader("Filme 0 Gy (background)", type=['tif','tiff','png','jpg'], key="g_0")

        curva_file = st.file_uploader("Curva de calibração (.json)", type=['json'], key="g_curva")
        if 'curva_calibracao' in st.session_state and st.checkbox("Usar curva da sessão", value=True):
            curva_gamma = st.session_state['curva_calibracao']
        else:
            curva_gamma = None

        if curva_file:
            curva_gamma = json.load(curva_file)

        if up_irr and up_0gy and curva_gamma:
            with st.spinner("Gerando mapa..."):
                ...
                # Detectar direção
                media_bordas = np.mean([c_irr[0,:], c_irr[-1,:], c_irr[:,0], c_irr[:,-1]])
                media_centro = np.mean(c_irr[c_irr.shape[0]//3:2*c_irr.shape[0]//3, c_irr.shape[1]//3:2*c_irr.shape[1]//3])
                adc_aumenta = media_bordas > media_centro
                ...
                # Curva
                tipo = curva_gamma.get('tipo_fitting', 'polynomial2')
                k1, k2 = curva_gamma.get('K1', 0), curva_gamma.get('K2', 0)
                ...
                st.success(f"✅ Mapa gerado: {dose_film.shape[1]}×{dose_film.shape[0]} px")
        else:
            st.warning("Envie filme irradiado + 0 Gy + curva")
            st.stop()

    # Resolução do filme
    if res_film_mm is None:
        res_film_mm = st.number_input("Resolução do filme (mm/pixel)", 0.01, 10.0, 0.35, 0.01, format="%.3f")
        dpi = st.number_input("Ou DPI", 10, 2400, 72)
        if dpi > 0:
    ...
    arq_tps = st.file_uploader("Arquivo TPS (.ALL, .csv, .dcm, .png)", type=['all','csv','txt','dcm','png','jpg','tif'], key="g_tps")
    ...
    if arq_tps:
        ...
        st.info("Envie o arquivo TPS")
        st.stop()

    # ========== PASSO 2: INTERPOLAR TPS PARA RESOLUÇÃO DO FILME ==========
    st.markdown("---")
    st.subheader("2️⃣ Interpolação do TPS para Resolução do Filme")

    # O TPS (baixa resolução) deve ser interpolado para a resolução do filme
    # Isso é o padrão da indústria (DoseLab Pro, OmniPro)
    from scipy.ndimage import zoom

    zoom_y = res_tps_mm / res_film_mm  # TPS → filme
    zoom_x = res_tps_mm / res_film_mm

    dose_tps_interp = zoom(dose_tps, (zoom_y, zoom_x), order=1, mode='nearest')
    res_tps_interp_mm = res_film_mm  # agora na mesma resolução do filme

    st.info(f"TPS interpolado: {dose_tps_interp.shape[1]}×{dose_tps_interp.shape[0]} px @ {res_tps_interp_mm:.3f} mm/px")

    # ========== PASSO 3: PREVIEW DOS DOIS MAPAS (MESMA ESCALA) ==========
    st.markdown("---")
    st.subheader("3️⃣ Preview dos Mapas")

    try:
        fig_prev, ax_prev = plt.subplots(1, 2, figsize=(14, 6))

        vmax = max(float(np.max(dose_film)), float(np.max(dose_tps_interp)))

        im0 = ax_prev[0].imshow(dose_film, cmap='jet', vmin=0, vmax=vmax, origin='lower')
        ax_prev[0].set_title(f"Filme\n{dose_film.shape[1]}×{dose_film.shape[0]} | {res_film_mm:.2f} mm/px")
        ax_prev[0].axis('off')
        plt.colorbar(im0, ax=ax_prev[0], fraction=0.046)

        im1 = ax_prev[1].imshow(dose_tps_interp, cmap='jet', vmin=0, vmax=vmax, origin='lower')
        ax_prev[1].set_title(f"TPS Interpolado\n{dose_tps_interp.shape[1]}×{dose_tps_interp.shape[0]} | {res_tps_interp_mm:.2f} mm/px")
        ax_prev[1].axis('off')
    ...
    # ========== PASSO 4: REGISTRO COM FLIP ==========
    st.markdown("---")
    st.subheader("4️⃣ Registro e Ajuste de Orientação")

    # Opções de orientação
    flip_h = st.checkbox("Inverter Horizontalmente (Flip X)", value=False)
    flip_v = st.checkbox("Inverter Verticalmente (Flip Y)", value=False)

    # Aplicar flip ao filme se necessário
    dose_film_reg = np.copy(dose_film)
    if flip_h:
        dose_film_reg = np.fliplr(dose_film_reg)
    if flip_v:
        dose_film_reg = np.flipud(dose_film_reg)
    ...
    if st.button("🔧 Registro Automático", type="primary"):
        with st.spinner("Alinhando..."):
            try:
                reg = auto_register_dose_maps(dose_film_reg, dose_tps_interp, res_film_mm, res_tps_interp_mm, allow_rotation=False)
                tx, ty = reg['tx_mm'], reg['ty_mm']
                st.session_state['reg_tx'] = tx
                st.session_state['reg_ty'] = ty
                st.success(f"✅ Alinhado: X={tx:.1f}mm, Y={ty:.1f}mm | Score: {reg['correlation_score']:.3f}")
            except Exception as e:
                st.warning(f"Registro automático falhou: {e}")

    with st.expander("⚙️ Ajuste Manual"):
        tx_default = max(-200.0, min(200.0, st.session_state.get('reg_tx', 0.0)))
        ty_default = max(-200.0, min(200.0, st.session_state.get('reg_ty', 0.0)))
        tx = st.slider("X (mm)", -200.0, 200.0, tx_default, 0.5)
        ty = st.slider("Y (mm)", -200.0, 200.0, ty_default, 0.5)

    # ========== PASSO 5: OVERLAY ==========
    st.markdown("---")
    st.subheader("5️⃣ Sobreposição (Overlay)")

    try:
        # Aplicar transformação ao TPS interpolado
        shift_px_x = int(tx / res_tps_interp_mm)
        shift_px_y = int(ty / res_tps_interp_mm)
        from scipy.ndimage import shift
        tps_shifted = shift(dose_tps_interp, (shift_px_y, shift_px_x), order=1, mode='constant', cval=0)

        # Criar figura de overlay
        fig_over, ax_over = plt.subplots(1, 3, figsize=(18, 5))

        # Filme (base)
        im0 = ax_over[0].imshow(dose_film_reg, cmap='gray', origin='lower')
        ax_over[0].set_title("Filme")
        ax_over[0].axis('off')
        plt.colorbar(im0, ax=ax_over[0], fraction=0.046)

        # TPS sobreposto
        ax_over[1].imshow(dose_film_reg, cmap='gray', origin='lower', alpha=0.5)
        ax_over[1].imshow(tps_shifted, cmap='jet', origin='lower', alpha=0.5)
        ax_over[1].set_title("Sobreposição Filme + TPS")
        ax_over[1].axis('off')

        # Contornos
        levels = [0.5 * np.max(dose_film_reg), 0.8 * np.max(dose_film_reg)]
        ax_over[2].imshow(dose_film_reg, cmap='gray', origin='lower', alpha=0.4)
        cs1 = ax_over[2].contour(dose_film_reg, levels=levels, colors='yellow', linewidths=2, origin='lower')
        cs2 = ax_over[2].contour(tps_shifted, levels=levels, colors='cyan', linewidths=1.5, linestyles='--', origin='lower')
        ax_over[2].set_title("Contornos: Filme (amarelo) + TPS (cyan)")
        ax_over[2].axis('off')

        plt.tight_layout()
        st.pyplot(fig_over)
    except Exception as e:
        st.error(f"Erro no overlay: {e}")

    # ========== PASSO 6: PARÂMETROS GAMMA ==========
    st.markdown("---")
    st.subheader("6️⃣ Parâmetros Gamma")
    ...
    global_norm = st.checkbox("Normalização Global", value=True)

    # ========== PASSO 7: CALCULAR GAMMA COM ROI ==========
    st.markdown("---")
    if st.button("🚀 CALCULAR GAMMA", type="primary"):
        with st.spinner("Calculando..."):
            try:
                # Aplicar registro ao TPS
                tps_for_gamma = shift(dose_tps_interp, (int(ty/res_tps_interp_mm), int(tx/res_tps_interp_mm)), order=1, mode='constant', cval=0)

                # Criar ROI: apenas onde TPS tem dose > threshold
                roi_mask = tps_for_gamma > (thresh / 100.0 * np.max(tps_for_gamma))

                # Garantir mesmo shape
                if tps_for_gamma.shape != dose_film_reg.shape:
                    st.error(f"Shapes diferentes: TPS {tps_for_gamma.shape} vs Filme {dose_film_reg.shape}")
                    st.stop()

                # Calcular gamma
                gamma_res = gamma_analysis_2d(
                    reference_dose=tps_for_gamma,
                    evaluation_dose=dose_film_reg,
                    reference_resolution_mm=res_tps_interp_mm,
                    evaluation_resolution_mm=res_film_mm,
    ...
    if 'gamma_res' in st.session_state:
        ...
        try:
            ...
            im0 = ax_r[0,0].imshow(tps_for_gamma, cmap='jet', origin='lower')
            ax_r[0,0].set_title(f"TPS (Registrado)\nMax: {tps_for_gamma.max():.2f} Gy")
            ax_r[0,0].axis('off')
            plt.colorbar(im0, ax=ax_r[0,0], fraction=0.046)

            im1 = ax_r[0,1].imshow(dose_film_reg, cmap='jet', origin='lower')
            ax_r[0,1].set_title(f"Filme\nMax: {dose_film_reg.max():.2f} Gy")
            ax_r[0,1].axis('off')
            plt.colorbar(im1, ax=ax_r[0,1], fraction=0.046)

            gamma_map = g['gamma_map']
            gamma_display = np.where(g['valid_mask'], gamma_map, np.nan)
            im2 = ax_r[1,0].imshow(gamma_display, cmap='RdYlGn_r', vmin=0, vmax=2, origin='lower')
            ax_r[1,0].set_title(f"Gamma | {g['passing_rate']:.1f}% pass")
            ax_r[1,0].axis('off')
            plt.colorbar(im2, ax=ax_r[1,0], fraction=0.046)

            # Overlay final
            ax_r[1,1].imshow(dose_film_reg, cmap='gray', origin='lower', alpha=0.4)
            levels = [0.5 * np.max(dose_film_reg), 0.8 * np.max(dose_film_reg)]
            ax_r[1,1].contour(dose_film_reg, levels=levels, colors='yellow', linewidths=2, origin='lower')
            ax_r[1,1].contour(tps_for_gamma, levels=levels, colors='cyan', linewidths=1.5, linestyles='--', origin='lower')
