...
try:
    from chromis_report import ChromisReport, save_plot_to_file, gerar_relatorio
    HAS_REPORT = True
except ImportError:
    HAS_REPORT = False
...
if HAS_REPORT:
    st.info("Gere um relatório profissional em PDF com todas as análises realizadas nesta sessão.")
    
    # Verificar quais análises foram feitas
    analises_feitas = []
    if 'curva_calibracao' in st.session_state:
        analises_feitas.append("✅ Calibração")
    if 'mapa_dose_gerado' in st.session_state:
        analises_feitas.append("✅ Mapa de Dose")
    if 'gamma_result' in st.session_state:
        analises_feitas.append("✅ Análise Gamma")
    
    if analises_feitas:
        st.write("**Análises disponíveis no relatório:**")
        for a in analises_feitas:
            st.write(a)
        
        if st.button("📄 GERAR RELATÓRIO PDF", type="primary", key="btn_relatorio"):
            with st.spinner("Gerando relatório profissional..."):
                try:
                    ...
                    pdf_path = gerar_relatorio(analises)
        ...
        st.warning("⚠️ Nenhuma análise foi realizada nesta sessão. Execute pelo menos uma análise para gerar o relatório.")
else:
    st.info("📄 Módulo de relatório não disponível. Instale: `pip install fpdf2`")
