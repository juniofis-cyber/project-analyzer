...
# ==================== ANÁLISE GAMMA (REESCRITA v11) ====================
    ...
    # ============================================================
    # PARTE 5: ROI (REGIÃO DE INTERESSE)
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 5. Região de Interesse (ROI)")
    st.info("A ROI define onde o gamma será calculado. Fora da ROI, os pixels são ignorados. TG-218 recomenda threshold de 10-20% da dose máxima.")

    roi_mode = st.radio("Modo de ROI", ["Automático (threshold de dose)", "Manual (coordenadas)"], index=0, key="gg_roi_mode_v11")

    roi_mask = None

    if roi_mode == "Automático (threshold de dose)":
        threshold_roi = st.slider("Threshold de dose (% do máx)", 1, 50, 10, 1, key="gg_roi_thresh",
                                   help="Pixels com dose abaixo deste percentual do máximo são excluídos da análise. TG-218 recomenda 10-20%.")

        roi_mask, bbox_roi = detect_film_roi(film_aligned, threshold_percent=float(threshold_roi))

        # Visualizar ROI
        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(film_aligned, cmap='gray', alpha=0.3)
        roi_disp = np.where(roi_mask, film_aligned, np.nan)
        ...
        ax_roi.set_title(f"ROI: {w}×{h} px | {100*np.sum(roi_mask)/roi_mask.size:.1f}% da área | Threshold: {threshold_roi}%")
        ax_roi.axis('off')
        ...
        # ROI manual
        h_img, w_img = film_aligned.shape
        st.info(f"Dimensão da imagem: {w_img}×{h_img} px")

        col_r1, col_r2 = st.columns(2)
        with col_r1:
            roi_x = st.number_input("X inicial", 0, w_img-1, int(w_img*0.1), key="gg_rx_v11")
            roi_y = st.number_input("Y inicial", 0, h_img-1, int(h_img*0.1), key="gg_ry_v11")
        with col_r2:
            roi_w = st.number_input("Largura", 1, w_img-roi_x, int(w_img*0.8), key="gg_rw_v11")
            roi_h = st.number_input("Altura", 1, h_img-roi_y, int(h_img*0.8), key="gg_rh_v11")

        roi_mask = np.zeros_like(film_aligned, dtype=bool)
        roi_mask[roi_y:roi_y+roi_h, roi_x:roi_x+roi_w] = True

        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(film_aligned, cmap='gray', alpha=0.3)
        roi_disp = np.where(roi_mask, film_aligned, np.nan)
        im_roi = ax_roi.imshow(roi_disp, cmap='jet', alpha=0.7)
        rect = plt.Rectangle((roi_x, roi_y), roi_w, roi_h, fill=False, edgecolor='red', linewidth=3)
        ax_roi.add_patch(rect)
        ax_roi.set_title(f"ROI Manual: {roi_w}×{roi_h} px")
        ax_roi.axis('off')
        plt.colorbar(im_roi, ax=ax_roi, fraction=0.046)
        st.pyplot(fig_roi)

    st.info(f"📊 ROI: **{np.sum(roi_mask)} pixels** ({100*np.sum(roi_mask)/roi_mask.size:.1f}% da área total)")
