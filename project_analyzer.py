...
from gamma_engine import resize_to_shape, apply_shift, gamma_analysis
...
# ==================== ANÁLISE GAMMA (VERSÃO CORRETA) ====================



# ==================== ANÁLISE GAMMA (VERSÃO FINAL) ====================
    ...
    if 'mapa_dose_filme' in st.session_state:
        usar_existente = st.checkbox("Usar mapa gerado em 'Vários filmes'", value=True)
        if usar_existente:
            dose_film = st.session_state['mapa_dose_filme']
            res_film_mm = st.session_state.get('res_film_mm', 0.35)
            st.success(f"✅ Filme: {dose_film.shape[1]}×{dose_film.shape[0]} px")
    ...
    if dose_film is None:
        ...
        with col2:
            up_0gy = st.file_uploader("Filme 0 Gy (BACKGROUND)", type=['tif','tiff','png','jpg'], key="gg_0")
    ...
    if res_film_mm is None:
        res_film_mm = st.number_input("Resolução do filme (mm/px)", 0.01, 10.0, 0.35, 0.01, format="%.3f")
        dpi = st.number_input("Ou DPI do scanner", 10, 2400, 72)
        if dpi > 0:
    ...
    arq_tps = st.file_uploader("Arquivo TPS (.ALL, .csv, .png)", type=['all','csv','txt','png','jpg','tif'], key="gg_tps")
    ...
    from gamma_engine import resize_to_shape

    dose_tps = resize_to_shape(dose_tps_raw, dose_film.shape)

    st.info(f"TPS redimensionado: {dose_tps.shape[1]}×{dose_tps.shape[0]} px (era {dose_tps_raw.shape[1]}×{dose_tps_raw.shape[0]})")
    ...
    ax_prev[0].set_title(f"Filme\n{dose_film.shape[1]}×{dose_film.shape[0]} | {res_film_mm:.3f} mm/px")
    ax_prev[0].axis('off')
    plt.colorbar(im0, ax=ax_prev[0], fraction=0.046)

    im1 = ax_prev[1].imshow(dose_tps, cmap='jet', vmin=0, vmax=vmax, origin='lower')
    ax_prev[1].set_title(f"TPS Redimensionado\n{dose_tps.shape[1]}×{dose_tps.shape[0]} | {res_film_mm:.3f} mm/px")
    ax_prev[1].axis('off')
    ...
    # ============================================================
    # PARTE 5: REGISTRO (SHIFT DO TPS)
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 5. Registro (Alinhamento)")

    # Opções de orientação
    col_flip1, col_flip2 = st.columns(2)
    with col_flip1:
        flip_h = st.checkbox("Inverter Horizontalmente", value=False, key="gg_flip_h")
    with col_flip2:
        flip_v = st.checkbox("Inverter Verticalmente", value=False, key="gg_flip_v")

    # Aplicar flip ao filme
    ...
    if st.button("🔧 Registro Automático", type="primary"):
        with st.spinner("Alinhando..."):
            try:
                from gamma_engine import resize_to_shape
                from registration import auto_register_dose_maps
                reg = auto_register_dose_maps(dose_film_aligned, dose_tps, res_film_mm, res_film_mm, allow_rotation=False)
    ...
    with st.expander("⚙️ Ajuste Manual"):
        tx = st.slider("X (mm)", -200.0, 200.0, st.session_state.get('gg_tx', 0.0), 0.5, key="gg_s_x")
        ty = st.slider("Y (mm)", -200.0, 200.0, st.session_state.get('gg_ty', 0.0), 0.5, key="gg_s_y")

    # ============================================================
    # PARTE 6: APLICAR SHIFT AO TPS E MOSTRAR OVERLAY
    # ============================================================
    st.markdown("---")
    st.subheader("📐 6. Sobreposição (Overlay)")

    from gamma_engine import apply_shift

    dose_tps_shifted = apply_shift(dose_tps, tx, ty, res_film_mm)

    fig_over, ax_over = plt.subplots(1, 3, figsize=(18, 5))

    # Filme
    im0 = ax_over[0].imshow(dose_film_aligned, cmap='gray', origin='lower')
    ax_over[0].set_title("Filme")
    ax_over[0].axis('off')
    plt.colorbar(im0, ax=ax_over[0], fraction=0.046)

    # TPS Shifted
    im1 = ax_over[1].imshow(dose_tps_shifted, cmap='jet', origin='lower')
    ax_over[1].set_title(f"TPS Alinhado\nShift: ({tx:.1f}, {ty:.1f}) mm")
    ax_over[1].axis('off')
    plt.colorbar(im1, ax=ax_over[1], fraction=0.046)

    # Sobreposição REAL
    ax_over[2].imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.5)
    ax_over[2].imshow(dose_tps_shifted, cmap='jet', origin='lower', alpha=0.5)
    ax_over[2].set_title("Sobreposição Filme + TPS")
    ax_over[2].axis('off')
    ...
    # Contornos
    st.subheader("📐 Contornos de Isodose")
    fig_cont, ax_cont = plt.subplots(figsize=(8, 6))
    ax_cont.imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.5)

    levels = [0.5 * np.max(dose_film_aligned), 0.8 * np.max(dose_film_aligned)]
    ax_cont.contour(dose_film_aligned, levels=levels, colors='yellow', linewidths=2, origin='lower')
    ax_cont.contour(dose_tps_shifted, levels=levels, colors='cyan', linewidths=2, linestyles='--', origin='lower')
    ax_cont.set_title("Filme (amarelo) vs TPS (cyan tracejado)")
    ax_cont.axis('off')

    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], color='yellow', lw=2, label='Filme'),
        Line2D([0], [0], color='cyan', lw=2, linestyle='--', label='TPS')
    ]
    ax_cont.legend(handles=legend_elements, loc='upper right')
    st.pyplot(fig_cont)

    # ============================================================
    # PARTE 7: DELIMITAR ROI
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 7. Região de Interesse (ROI)")

    roi_threshold_pct = st.slider("Threshold do ROI (% da dose máxima)", 5, 50, 10, 5,
                                   help="A análise gamma será feita apenas onde a dose do TPS é maior que este percentual")

    roi_threshold = (roi_threshold_pct / 100.0) * np.max(dose_tps_shifted)
    roi_mask = dose_tps_shifted > roi_threshold

    # Mostrar ROI
    fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
    ax_roi.imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.3)
    roi_display = np.where(roi_mask, dose_tps_shifted, np.nan)
    im_roi = ax_roi.imshow(roi_display, cmap='jet', origin='lower', alpha=0.7)
    ax_roi.set_title(f"ROI: dose > {roi_threshold_pct}% ({roi_threshold:.2f} Gy)")
    ax_roi.axis('off')
    ...
    global_norm = st.checkbox("Normalização Global", value=True, key="gg_glob")
    ...
    if st.button("🚀 CALCULAR GAMMA", type="primary"):
        with st.spinner("Calculando gamma... (pode levar 1-2 min)"):
            try:
                from gamma_engine import gamma_analysis

                gamma_res = gamma_analysis(
                    dose_film=dose_film_aligned,
                    dose_tps_resized=dose_tps_shifted,
                    res_mm=res_film_mm,
                    dose_percent=float(dd),
                    dta_mm=float(dta),
                    threshold_percent=float(roi_threshold_pct),
    ...
    if 'gg_result' in st.session_state:
        ...
        ax_r[1,0].set_title(f"Gamma | {g['passing_rate']:.1f}% pass")
        ax_r[1,0].axis('off')
        ...
        ax_r[1,1].set_title("Overlay: Filme + TPS")
        ax_r[1,1].axis('off')
        ...
        st.write(f"- Threshold ROI: **{roi_threshold_pct}%**")
