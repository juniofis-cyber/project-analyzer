...
# Overlay de isodoses filme vs TPS + % de coincidência
    ...
    st.info("Sobreposição de mapas de isodose do filme radiocrômico com o TPS. Analise a coincidência das curvas de isodose e calcule o percentual de sobreposição.")
    ...
    with col1:
        ...
        res_mm = 25.4 / dpi
    with col2:
        isodoses = st.multiselect("Isodoses para comparar (%)", [50, 80, 90, 95, 100], default=[90, 100], key="iso_levels")
    with col3:
    ...
    if up_irr and up_tps and up_0gy and curva_up:
        ...
        if st.button("🚀 CALCULAR COINCIDÊNCIA", type="primary", key="iso_calc"):
            ...
            try:
                from gamma_engine import resize_to_shape, register_with_flip, compare_isodose, apply_shift
                
                # 1. Carregar imagens
                progress_bar.progress(5, text="1/8 - Carregando imagens...")
                img_irr, _ = carregar_imagem_preservando_bits(up_irr)
                img_0gy, _ = carregar_imagem_preservando_bits(up_0gy)
                
                # Verificar tamanho - redimensionar se muito grande (otimizacao Cloud)
                max_dim = 800
                if img_irr.shape[0] > max_dim or img_irr.shape[1] > max_dim:
                    st.info(f"Redimensionando imagens de {img_irr.shape[:2]} para {max_dim}px max (otimizacao)...")
                    scale = max_dim / max(img_irr.shape[0], img_irr.shape[1])
                    new_h, new_w = int(img_irr.shape[0] * scale), int(img_irr.shape[1] * scale)
                    img_irr = np.array(Image.fromarray(img_irr.astype(np.uint8)).resize((new_w, new_h), Image.LANCZOS))
                    img_0gy = np.array(Image.fromarray(img_0gy.astype(np.uint8)).resize((new_w, new_h), Image.LANCZOS))
                    
                    # 2. Processar filme
                    progress_bar.progress(15, text="2/8 - Detectando e cortando filmes...")
                    filme_irr = cortar_filme_unico(img_irr)
                    filme_0gy = cortar_filme_unico(img_0gy)
                    ...
                    progress_bar.progress(25, text="3/8 - Removendo fundo branco...")
                    filme_irr_sf, _, _ = remover_fundo_branco(filme_irr)
                    filme_0gy_sf, _, _ = remover_fundo_branco(filme_0gy)
                    
                    progress_bar.progress(35, text="4/8 - Calculando dose do filme...")
                    red_0gy = canal_vermelho(filme_0gy_sf)
                    pv0 = float(np.mean(red_0gy))
                    ...
                    # 5. Ler TPS
                    progress_bar.progress(50, text="5/8 - Lendo arquivo TPS...")
                    up_tps.seek(0)
                    tps_data = up_tps.read()
                    tps_dist = read_tps(tps_data, filename=up_tps.name)
                    dose_tps_raw = tps_dist.dose
                    st.info(f"TPS: {dose_tps_raw.shape[1]}x{dose_tps_raw.shape[0]} px, max={dose_tps_raw.max():.2f} Gy")
                    
                    # 6. Redimensionar TPS para filme
                    progress_bar.progress(60, text="6/8 - Alinhando resolucoes...")
                    dose_tps = resize_to_shape(dose_tps_raw, dose_film.shape)
                    
                    # 7. Registro automático
                    progress_bar.progress(70, text="7/8 - Registro automatico (flip + translacao)...")
                    film_aligned, best_flip, tx, ty, score = register_with_flip(
                        dose_film, dose_tps, res_mm
                    )
                    tps_shifted = apply_shift(dose_tps, tx, ty, res_mm)
                    
                    st.success(f"✅ Registro: flip={best_flip}, shift=({tx:.1f}mm, {ty:.1f}mm), score={score:.3f}")
                    
                    # 8. Comparação de isodose
                    progress_bar.progress(85, text="8/8 - Comparando isodoses...")
                    iso_result = compare_isodose(
                        dose_film=film_aligned,
                        dose_tps=tps_shifted,
                        isodose_levels=[int(x) for x in isodoses],
                        tolerance_mm=tol_mm,
                        res_mm=res_mm,
                    )
                    ...
                    df = pd.DataFrame([
                        {
                            ...
                            'Pixels Filme': r['pixels_film'],
                            'Pixels TPS': r['pixels_tps'],
                        }
                    ...
                    for i, r in enumerate(iso_result['results']):
                        ...
                        cols[i].metric(
                            f"{r['level']}%", 
                            f"{r['coincidence']:.1f}%",
                            f"{status} {r['mean_distance_mm']:.1f}mm"
                        )
                    
                    # Gráfico de overlay
                    st.subheader("Overlay de Isodoses")
                    fig_iso, ax_iso = plt.subplots(1, 2, figsize=(14, 6))
                    
                    ax_iso[0].imshow(film_aligned, cmap='gray', alpha=0.3)
                    ax_iso[0].imshow(iso_result['overlay_film'])
                    ax_iso[0].set_title(f"Filme ({best_flip})")
                    ax_iso[0].axis('off')
                    
                    ax_iso[1].imshow(tps_shifted, cmap='gray', alpha=0.3)
                    ax_iso[1].imshow(iso_result['overlay_tps'])
                    ax_iso[1].set_title("TPS Alinhado")
                    ax_iso[1].axis('off')
                ...
                progress_bar.empty()
                st.error(f"❌ Erro de validacao: {e}")
            except MemoryError:
                progress_bar.empty()
                st.error("❌ Memoria insuficiente. Tente com imagens menores (reduza o DPI do scanner).")
            except Exception as e:
                progress_bar.empty()
...
# ============================================================
st.markdown("---")
st.caption("Chromis v3.1 | Dosimetria com Filmes Radiocrômicos EBT3/EBT4 | Análise Gamma TG-218")
