...
# === MÓDULOS PARA ANÁLISE GAMMA ===
from gamma_engine import resize_to_shape, apply_shift, register_with_flip, detect_film_roi, gamma_analysis, gamma_analysis_optimized
try:
...
# Funcao de relatorio diretamente aqui (evita dependencia de modulo externo)
from tps_parser import read_tps, DoseDistribution

# ========== FUNCAO RELATORIO PDF ==========

def gerar_relatorio_pdf(analises, info_paciente=None, output_path='/tmp/chromis_relatorio.pdf'):
    """Gera relatorio PDF profissional com graficos e fotos reais."""
    ...
    with PdfPages(output_path) as pdf:
        ...
        try:
            ...
            ax.imshow(logo, extent=[2.885, 2.885 + logo_w, 9.5, 9.5 + logo_h], aspect='auto', zorder=5)
        except:
            pass

        ax.text(4.135, 8.8, 'RELATORIO DE DOSIMETRIA', fontsize=24, fontweight='bold', ha='center', color='#1e3a5f', zorder=10)
        ax.text(4.135, 8.3, 'com Filmes Radiocromicos EBT3/EBT4', fontsize=14, ha='center', color='#64748b', zorder=10)

        rect = plt.Rectangle((1, 5.0), 6.27, 2.5, fill=True, facecolor='#f1f5f9', edgecolor='#1e3a5f', linewidth=2)
        ax.add_patch(rect)

        ax.text(1.3, 7.0, 'INFORMACOES GERAIS', fontsize=13, fontweight='bold', color='#1e3a5f')
        now = datetime.now().strftime('%d/%m/%Y %H:%M')
        y_info = 6.4
        for info in [f'Data/Hora: {now}', 'Software: Chromis v3.2', 'Autor: MACIEL, J. O.', 'Curriculo: http://lattes.cnpq.br/3347976525922556']:
            ax.text(1.3, y_info, info, fontsize=11, color='#374151')
            y_info -= 0.35

        ax.text(1.3, 4.0, 'ANALISES REALIZADAS', fontsize=13, fontweight='bold', color='#1e3a5f')
        y_anal = 3.4
        for nome, feita in [('Calibracao', analises.get('calibracao', {}).get('feita', False)), ('Mapa de Dose 2D', analises.get('dose_map', {}).get('feita', False)), ('Mapa de Isodoses', analises.get('isodose', {}).get('feita', False)), ('Analise Gamma', analises.get('gamma', {}).get('feita', False)), ('Comparacao de Isodose', analises.get('iso_compare', {}).get('feita', False))]:
            status, color = ('[OK]', '#059669') if feita else ('[  ]', '#9ca3af')
            ax.text(1.3, y_anal, f'{status} {nome}', fontsize=11, color=color)
            y_anal -= 0.35

        ax.text(4.135, 0.8, 'Desenvolvido por MACIEL, J. O. | Chromis v3.2', fontsize=9, ha='center', color='#9ca3af', style='italic')
        ...
        if analises.get('calibracao', {}).get('feita'):
            ...
            fig.suptitle('1. Curva de Calibracao', fontsize=16, fontweight='bold', color='#1e3a5f', y=0.98)
            ax1 = axes[0]
            curva = analises['calibracao'].get('curva', {})
            r2 = analises['calibracao'].get('r2', 0)
            try:
                nod_data = curva.get('dose_vs_nod', [])
                if nod_data and len(nod_data) > 0:
                    nod_vals = [p.get('nod', 0) for p in nod_data]
                    dose_vals = [p.get('dose', 0) for p in nod_data]
                    ax1.scatter(nod_vals, dose_vals, c='#2563eb', s=50, alpha=0.7, label='Dados experimentais')
                if 'a' in curva and 'b' in curva and 'c' in curva:
                    a, b, c = float(curva['a']), float(curva['b']), float(curva['c'])
                    nod_fit = np.linspace(0, max(nod_vals) if nod_vals else 1.5, 200)
                    dose_fit = a + b * nod_fit + c * nod_fit**2
                    ax1.plot(nod_fit, dose_fit, 'r-', linewidth=2, label=f'Ajuste (R2={r2:.4f})')
                elif 'K1' in curva and 'K2' in curva:
                    k1, k2 = float(curva['K1']), float(curva['K2'])
                    nod_fit = np.linspace(0, max(nod_vals) if nod_vals else 1.5, 200)
                    dose_fit = k1 * np.maximum(nod_fit, 0)**k2
                    ax1.plot(nod_fit, dose_fit, 'r-', linewidth=2, label=f'Ajuste (R2={r2:.4f})')
            except:
                ax1.text(0.5, 0.5, 'Curva de Calibracao', transform=ax1.transAxes, ha='center', va='center', fontsize=12, color='#9ca3af')
            ax1.set_xlabel('NOD', fontsize=11)
            ...
            ax2.axis('off')
            params = []
            for k, label in [('a', 'a'), ('b', 'b'), ('c', 'c'), ('K1', 'K1'), ('K2', 'K2'), ('tipo_fitting', 'Tipo')]:
                if k in curva:
                    params.append([label, f'{curva[k]}'])
            params.append(['R2', f'{r2:.4f}'])
            if params:
                table = ax2.table(cellText=params, colLabels=['Parametro', 'Valor'], loc='center', cellLoc='left')
        ...
        if analises.get('dose_map', {}).get('feita'):
            ...
            fig.suptitle('2. Mapa de Dose 2D', fontsize=16, fontweight='bold', color='#1e3a5f', y=0.98)
            ax1 = axes[0]
            max_dose = analises['dose_map'].get('max_dose', 0)
            ax1.text(0.5, 0.5, f'Mapa de Dose\nMax: {max_dose:.2f} Gy', transform=ax1.transAxes, ha='center', va='center', fontsize=14, color='#374151')
            ax1.set_title('Filme', fontsize=11)
            ax1.axis('off')
            ax2 = axes[1]
            ax2.axis('off')
            infos = [['Informacao', 'Valor'], ['Dose Maxima', f'{max_dose:.2f} Gy'], ['Tipo de Filme', 'EBT3/EBT4']]
            table = ax2.table(cellText=infos[1:], colLabels=infos[0], loc='center', cellLoc='left')
            table.auto_set_font_size(False)
        ...
        if analises.get('gamma', {}).get('feita'):
            ...
            fig.suptitle('3. Analise Gamma', fontsize=16, fontweight='bold', color='#1e3a5f', y=0.98)
            gg = analises['gamma']
            ax1 = axes[0, 0]
            ax1.axis('off')
            metrics = [['Metrica', 'Valor'], ['Criterio', f"{gg.get('dd', '?')}% / {gg.get('dta', '?')}mm"], ['Passing Rate', f"{gg.get('passing_rate', 0):.1f}%"], ['Gamma Medio', f"{gg.get('gamma_mean', 0):.3f}"], ['Gamma Maximo', f"{gg.get('gamma_max', 0):.3f}"]]
            table = ax1.table(cellText=metrics[1:], colLabels=metrics[0], loc='center', cellLoc='left')
            table.auto_set_font_size(False)
            table.set_fontsize(10)
            table.scale(1, 2)
        ...
        ax.axis('off')
        fig.patch.set_facecolor('white')
        ax.text(4.135, 10.5, 'RESUMO E CONCLUSAO', fontsize=18, fontweight='bold', ha='center', color='#1e3a5f')
        ax.plot([1, 7.27], [10.2, 10.2], color='#1e3a5f', linewidth=2)
        ...
        color = '#059669' if aprovado else '#dc2626'
        texto = 'APROVADO' if aprovado else 'REPROVADO'
        ax.text(4.135, 8.5, f'Status: {texto}', fontsize=22, fontweight='bold', ha='center', color=color)
        ax.plot([1, 3.5], [3.0, 3.0], color='#374151', linewidth=1)
        ax.text(1, 2.6, 'Assinatura do Fisico Medico', fontsize=10, color='#6b7280')
        ax.plot([4.5, 7], [3.0, 3.0], color='#374151', linewidth=1)
        ax.text(4.5, 2.6, 'Data', fontsize=10, color='#6b7280')
        pdf.savefig(fig, dpi=150, bbox_inches='tight', facecolor='white')
        plt.close(fig)

    return output_path

# ========== FIM FUNCAO RELATORIO ==========

st.set_page_config(page_title="Chromis", page_icon="🎞️", layout="wide")

# CSS customizado com paleta Chromis
st.markdown("""
<style>
    .main-header {
        display: flex;
...
# Logo Chromis no topo
st.image("Chromis_logo_principal.png", width=200)

# Descricao profissional
col1, col2 = st.columns([3, 1])
with col1:
    st.markdown("""
    **Chromis** é um software open-source para dosimetria com filmes radiocrômicos EBT3/EBT4 
    em radioterapia. Realiza calibração, gera mapas de dose 2D, perfis de isodoses e análise 
    gamma comparando filmes com TPS, seguindo as recomendações do AAPM TG-218.
    """)
with col2:
    st.markdown("Desenvolvido por **MACIEL, J. O.**")
    st.markdown("[🔗 Saiba mais sobre Lattes](http://lattes.cnpq.br/3347976525922556)")

# Info tecnica escondida em expander
with st.expander("ℹ️ Informacoes tecnicas do scanner"):
    st.markdown("""
    - O Chromis utiliza exclusivamente o **canal vermelho** (red channel) do scanner
    - Preserva o **bit-depth original** da imagem
    - Para scanners em **16-bit**, os valores de ADC devem estar na faixa de **~27.000 a ~52.000**
    - Canais verde e azul sao ignorados para maximizar a sensibilidade a radiacao
    """)

# Tipo de filme
tipo_filme = st.radio("**Tipo de Filme**", ["EBT3", "EBT4"], horizontal=True, key="tipo_filme_v2")

# Metodologia principal (3 modos)
st.markdown("---")
st.subheader("**Metodologia**")
metodologia = st.radio("", 
    ["Mapa de Dose/Isodose", "Análise Gamma", "Comparação de Isodose"], 
    horizontal=True, key="metod_v2")

# Sub-modo para Mapa de Dose/Isodose
sub_modo = None
if metodologia == "Mapa de Dose/Isodose":
    sub_modo = st.radio("**Modo**", ["Filme Único", "Múltiplos Filmes"], horizontal=True, key="submodo_v2")
st.markdown("---")

with st.sidebar:
    st.header("Configuracoes")
    dpi = st.number_input("DPI do Scanner", 1, 2400, 50)
    # Sidebar conforme metodologia
    if metodologia == "Mapa de Dose/Isodose":
        if sub_modo == "Filme Único":
            st.subheader("Modo Filme Único")
        ...
        elif sub_modo == "Múltiplos Filmes":
            st.subheader("Modo Múltiplos Filmes")
    ...
    elif metodologia == "Análise Gamma":
        st.subheader("Modo Análise Gamma")
        st.info("Gamma: filme vs TPS")
    elif metodologia == "Comparação de Isodose":
        st.subheader("Modo Comparação de Isodose")
        st.info("Overlay de isodoses")
    
    # Configurações comuns (scanner / curva)
    if metodologia == "Mapa de Dose/Isodose" and sub_modo == "Múltiplos Filmes":
        st.markdown("---")
    ...
    # Curva de calibração (para todos os modos)
    st.markdown("---")
...
# ==================== MAPA DE DOSE/ISODOSE ====================
# Sub-modos: Filme Único | Múltiplos Filmes

if metodologia == "Mapa de Dose/Isodose" and sub_modo == "Filme Único":
...
elif metodologia == "Mapa de Dose/Isodose" and sub_modo == "Múltiplos Filmes":
    st.header("Importar Filmes")
                        ...
                        if filme_mapa_m is not None:
                            ...
                            if st.button("🔬 GERAR MAPAS", type="primary", key="btn_mapa_m"):
                                with st.spinner("Calculando mapas..."):
                                    ...
                                    st.session_state["res_film_mm"] = 0.35
...
# ==================== ANÁLISE GAMMA (REESCRITA v11) ====================

elif metodologia == "Análise Gamma":
    st.header("🎯 Análise Gamma — Filme vs TPS")
    st.info("Compare a distribuição de dose medida pelo filme radiocrômico com a dose calculada pelo TPS. Baseado em TG-218.")

    # --- Estado da sessão para Gamma ---
    for key in ['gg_dose_film', 'gg_dose_tps', 'gg_res_mm', 'gg_film_aligned',
                'gg_tps_shifted', 'gg_best_flip', 'gg_tx', 'gg_ty', 'gg_result',
                'gg_roi_mask', 'gg_curva_usada', 'gg_filme_irr_proc', 'gg_fundo_removido']:
        if key not in st.session_state:
            st.session_state[key] = None

    # ============================================================
    # PARTE 1: MAPA DE DOSE DO FILME
    # ============================================================
    st.subheader("📤 1. Mapa de Dose do Filme")

    dose_film = None
    res_film_mm = None

    # Opção A: usar mapa existente do session_state
    if 'mapa_dose_filme' in st.session_state and st.session_state['mapa_dose_filme'] is not None:
        usar_existente = st.checkbox("Usar mapa gerado em 'Vários filmes'", value=True, key="gg_use_existing")
        if usar_existente:
            dose_film = st.session_state['mapa_dose_filme']
            res_film_mm = st.session_state.get('res_film_mm', None)
            st.success(f"✅ Mapa existente: {dose_film.shape[1]}×{dose_film.shape[0]} px")
            if res_film_mm:
                st.info(f"Resolução: {res_film_mm:.3f} mm/px")

    # Opção B: gerar mapa na aba Gamma com pipeline completo
    if dose_film is None:
        st.markdown("**Gerar mapa de dose completo:**")
        st.info("Envie o filme irradiado, o filme de 0 Gy (background) e a curva de calibração. O sistema detecta o filme automaticamente e gera o mapa de dose.")

        col1, col2 = st.columns(2)
        with col1:
            up_irr = st.file_uploader("📷 Filme IRRADIADO (TIFF/PNG)", type=['tif','tiff','png','jpg','jpeg'], key="gg_irr_v11")
        with col2:
            up_0gy = st.file_uploader("📷 Filme 0 Gy / BACKGROUND (TIFF/PNG)", type=['tif','tiff','png','jpg','jpeg'], key="gg_0gy_v11")

        # Curva de calibração
        curva_up = st.file_uploader("📊 Curva de calibração (.json)", type=['json'], key="gg_curva_v11")
        curva = None
        if curva_up:
            try:
                curva_raw = json.load(curva_up)
                # Normalizar formato
                if 'curva' in curva_raw:
                    curva = curva_raw['curva']
                    curva['tipo_fitting'] = curva_raw.get('tipo_fitting', 'polynomial2')
                elif 'a' in curva_raw or 'K1' in curva_raw:
                    curva = curva_raw
                else:
                    curva = curva_raw
                st.success("✅ Curva carregada do arquivo")
            except Exception as e:
                st.error(f"Erro ao carregar curva: {e}")
        elif 'curva_calibracao' in st.session_state:
            cc = st.session_state['curva_calibracao']
            if isinstance(cc, dict):
                if 'curva' in cc:
                    curva = cc['curva']
                else:
                    curva = cc
                st.info("Usando curva da sessão")

        # Entrada de DPI
        st.subheader("Configuração do Scanner")
        col_dpi1, col_dpi2 = st.columns(2)
        with col_dpi1:
            dpi_gamma = st.number_input("DPI do scanner", 10, 2400, 72, step=1, key="gg_dpi_v11")
        with col_dpi2:
            res_film_mm = st.number_input("Ou resolução (mm/px)", 0.001, 10.0, 25.4/dpi_gamma if dpi_gamma > 0 else 0.35, 0.001, format="%.4f", key="gg_res_v11")

        if dpi_gamma > 0:
            res_film_mm = 25.4 / dpi_gamma

        # Gerar mapa quando todos os arquivos estiverem prontos
        if up_irr and up_0gy and curva is not None:
            with st.spinner("Processando filme (pipeline completo)..."):
                try:
                    # 1. Carregar imagens preservando bit-depth
                    img_irr, info_irr = carregar_imagem_preservando_bits(up_irr)
                    img_0gy, info_0gy = carregar_imagem_preservando_bits(up_0gy)

                    st.info(f"Irradiado: {info_irr['dtype']} {info_irr['shape']} | 0 Gy: {info_0gy['dtype']} {info_0gy['shape']}")

                    # 2. Detectar e cortar filmes
                    filme_irr = cortar_filme_unico(img_irr)
                    filme_0gy = cortar_filme_unico(img_0gy)

                    if filme_irr is None:
                        st.error("❌ Não foi possível detectar o filme irradiado. Verifique a imagem.")
                        st.stop()
                    if filme_0gy is None:
                        st.error("❌ Não foi possível detectar o filme 0 Gy. Verifique a imagem.")
                        st.stop()

                    st.success(f"✅ Filme irradiado: {filme_irr.shape[1]}×{filme_irr.shape[0]} px")
                    st.success(f"✅ Filme 0 Gy: {filme_0gy.shape[1]}×{filme_0gy.shape[0]} px")

                    # 3. Remover fundo branco do scanner
                    filme_irr_sf, removeu_irr, _ = remover_fundo_branco(filme_irr)
                    filme_0gy_sf, removeu_0gy, _ = remover_fundo_branco(filme_0gy)

                    # 4. Preview dos filmes detectados
                    col_prev1, col_prev2 = st.columns(2)
                    with col_prev1:
                        st.image(visualizar_filme0_preview(filme_irr_sf), caption="Filme Irradiado (sem fundo)", use_container_width=True)
                    with col_prev2:
                        st.image(visualizar_filme0_preview(filme_0gy_sf), caption="Filme 0 Gy (sem fundo)", use_container_width=True)

                    # 5. Calcular PV0 (média do canal vermelho do filme 0 Gy)
                    red_0gy = canal_vermelho(filme_0gy_sf)
                    pv0 = float(np.mean(red_0gy))
                    st.info(f"PV0 (referência 0 Gy): {pv0:.1f}")

                    # 6. Detectar direção do scanner
                    red_irr = canal_vermelho(filme_irr_sf)
                    adc_media_irr = float(np.mean(red_irr))
                    adc_aumenta_com_dose = adc_media_irr > pv0

                    if adc_aumenta_com_dose:
                        st.info("📊 Scanner: ADC aumenta com dose (refletância/IBA)")
                    else:
                        st.info("📊 Scanner: ADC diminui com dose (transmissão padrão)")

                    # 7. Preparar curva no formato correto para calcular_mapa_dose
                    curva_dict = {}
                    if 'a' in curva and 'b' in curva and 'c' in curva:
                        curva_dict = {'a': float(curva['a']), 'b': float(curva['b']), 'c': float(curva['c'])}
                    elif 'K1' in curva and 'K2' in curva:
                        curva_dict = {'K1': float(curva['K1']), 'K2': float(curva['K2'])}
                    else:
                        # Tentar extrair de outros formatos
                        for k in ['a', 'b', 'c', 'K1', 'K2']:
                            if k in curva:
                                curva_dict[k] = float(curva[k])

                    # 8. Calcular mapa de dose usando a função TESTADA
                    dose_film = calcular_mapa_dose(filme_irr_sf, pv0, curva_dict, adc_aumenta_com_dose)

                    # Limpar valores inválidos
                    dose_film = np.where(np.isfinite(dose_film), dose_film, 0)
                    dose_film = np.clip(dose_film, 0, None)

                    st.session_state['gg_dose_film'] = dose_film
                    st.session_state['gg_res_mm'] = res_film_mm
                    st.session_state['gg_filme_irr_proc'] = filme_irr_sf
                    st.session_state['gg_curva_usada'] = curva_dict

                    st.success(f"✅ Mapa de dose gerado: {dose_film.shape[1]}×{dose_film.shape[0]} px | Max: {dose_film.max():.2f} Gy | Média: {dose_film.mean():.2f} Gy")
            ...
            st.warning("⚠️ Envie: filme irradiado + filme 0 Gy + curva de calibração")
            st.stop()

    # Confirmar resolução
    if res_film_mm is None:
        res_film_mm = st.number_input("Resolução do filme (mm/px)", 0.001, 10.0, 0.35, 0.001, format="%.4f", key="gg_res_fallback")

    st.info(f"📐 Resolução: **{res_film_mm:.4f} mm/px** ({25.4/res_film_mm:.0f} DPI)")

    # ============================================================
    # PARTE 2: UPLOAD DO TPS
    # ============================================================
    st.markdown("---")
    st.subheader("📤 2. Dose do TPS")

    arq_tps = st.file_uploader("Arquivo TPS (.ALL do Eclipse, .csv, .dcm, .png)", type=['all','csv','txt','dcm','png','jpg','tif','tiff'], key="gg_tps_v11")
    ...
    with st.spinner("Lendo arquivo TPS..."):
        try:
            ...
            dose_tps_raw = tps_dist.dose
            st.success(f"✅ TPS: {dose_tps_raw.shape[1]}×{dose_tps_raw.shape[0]} px | Max: {dose_tps_raw.max():.2f} Gy | Fonte: {tps_dist.source}")
        except Exception as e:
    ...
    # ============================================================
    # PARTE 3: REDIMENSIONAR TPS PARA MESMO SHAPE DO FILME
    # ============================================================
    st.markdown("---")
    st.subheader("🔧 3. Alinhamento de Resolução")

    dose_tps_resized = resize_to_shape(dose_tps_raw, dose_film.shape)
    st.info(f"TPS redimensionado: {dose_tps_resized.shape[1]}×{dose_tps_resized.shape[0]} px (era {dose_tps_raw.shape[1]}×{dose_tps_raw.shape[0]})")

    # Preview lado a lado
    fig_prev, ax_prev = plt.subplots(1, 2, figsize=(14, 6))
    vmax = max(float(np.max(dose_film)), float(np.max(dose_tps_resized)))
    im0 = ax_prev[0].imshow(dose_film, cmap='jet', vmin=0, vmax=vmax)
    ax_prev[0].set_title(f"Filme\n{dose_film.shape[1]}×{dose_film.shape[0]}")
    ax_prev[0].axis('off')
    plt.colorbar(im0, ax=ax_prev[0], fraction=0.046)
    im1 = ax_prev[1].imshow(dose_tps_resized, cmap='jet', vmin=0, vmax=vmax)
    ax_prev[1].set_title(f"TPS\n{dose_tps_resized.shape[1]}×{dose_tps_resized.shape[0]}")
    ax_prev[1].axis('off')
    plt.colorbar(im1, ax=ax_prev[1], fraction=0.046)
    plt.tight_layout()
    st.pyplot(fig_prev)

    # ============================================================
    # PARTE 4: REGISTRO AUTOMÁTICO COM FLIP
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 4. Registro Automático (Flip + Translação)")
    st.info("O sistema testa 4 orientações do filme (normal, flip H, flip V, flip HV) e encontra a melhor por correlação cruzada.")

    # Estado do registro
    film_aligned = st.session_state.get('gg_film_aligned')
    tps_shifted = st.session_state.get('gg_tps_shifted')
    best_flip = st.session_state.get('gg_best_flip', 'none')
    tx = st.session_state.get('gg_tx', 0.0)
    ty = st.session_state.get('gg_ty', 0.0)

    if st.button("🔧 Executar Registro Automático", type="primary", key="gg_btn_reg"):
        with st.spinner("Testando 4 orientações... (pode levar alguns segundos)"):
            try:
                fa, bf, tx_auto, ty_auto, score = register_with_flip(
                    dose_film, dose_tps_resized, res_film_mm
                )
                # Aplicar translação ao TPS
                ts = apply_shift(dose_tps_resized, tx_auto, ty_auto, res_film_mm)

                st.session_state['gg_film_aligned'] = fa
                st.session_state['gg_tps_shifted'] = ts
                st.session_state['gg_best_flip'] = bf
                st.session_state['gg_tx'] = tx_auto
                st.session_state['gg_ty'] = ty_auto
                st.session_state['gg_dose_film'] = dose_film
                st.session_state['gg_dose_tps'] = dose_tps_resized
                ...
                st.success(f"✅ Melhor orientação: **{bf}** | Shift: X={tx:.1f}mm, Y={ty:.1f}mm | Score: {score:.3f}")
                st.rerun()
            except Exception as e:
    ...
    # Se não há registro, usar dados originais
    if film_aligned is None:
        film_aligned = dose_film
        tps_shifted = dose_tps_resized

    # Ajuste manual (expandable)
    with st.expander("⚙️ Ajuste Manual de Registro (Flip + Translação)"):
        st.info("Ajuste fino se o registro automático não estiver perfeito.")

        col_m1, col_m2 = st.columns(2)
        with col_m1:
            tx_safe = float(tx) if tx is not None else 0.0
            ty_safe = float(ty) if ty is not None else 0.0
            tx = st.slider("Translação X (mm)", -100.0, 100.0, tx_safe, 0.5, key="gg_tx_manual")
            flip_h = st.checkbox("Flip Horizontal", value=(best_flip in ['h', 'hv']), key="gg_fh_manual")
        with col_m2:
            ty = st.slider("Translação Y (mm)", -100.0, 100.0, ty_safe, 0.5, key="gg_ty_manual")
            flip_v = st.checkbox("Flip Vertical", value=(best_flip in ['v', 'hv']), key="gg_fv_manual")

        if st.button("Aplicar Ajuste Manual", key="gg_btn_manual"):
            with st.spinner("Aplicando..."):
                # Aplicar flip ao filme original (não ao já flipado)
                fa_manual = np.copy(dose_film)
                if flip_h:
                    fa_manual = np.fliplr(fa_manual)
                if flip_v:
                    fa_manual = np.flipud(fa_manual)

                # Aplicar translação ao TPS
                ts_manual = apply_shift(dose_tps_resized, tx, ty, res_film_mm)

                st.session_state['gg_film_aligned'] = fa_manual
                st.session_state['gg_tps_shifted'] = ts_manual
                st.session_state['gg_best_flip'] = ('hv' if (flip_h and flip_v) else ('h' if flip_h else ('v' if flip_v else 'none')))
                st.session_state['gg_tx'] = tx
                st.session_state['gg_ty'] = ty

                film_aligned = fa_manual
                tps_shifted = ts_manual
                best_flip = st.session_state['gg_best_flip']
                st.success(f"✅ Ajuste aplicado: flip={best_flip}, tx={tx:.1f}mm, ty={ty:.1f}mm")
                st.rerun()

    # Preview do registro
    st.subheader("Preview do Registro")
    fig_reg, ax_reg = plt.subplots(1, 2, figsize=(14, 6))
    vmax_reg = max(float(np.max(film_aligned)), float(np.max(tps_shifted)))
    ax_reg[0].imshow(film_aligned, cmap='jet', vmin=0, vmax=vmax_reg)
    ax_reg[0].set_title(f"Filme ({best_flip})")
    ax_reg[0].axis('off')
    ax_reg[1].imshow(tps_shifted, cmap='jet', vmin=0, vmax=vmax_reg)
    ax_reg[1].set_title(f"TPS (shifted {tx:.1f}, {ty:.1f}mm)")
    ax_reg[1].axis('off')
    plt.tight_layout()
    st.pyplot(fig_reg)

    # ============================================================
    # PARTE 5: ROI (REGIÃO DE INTERESSE) — REESCRITO v13
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 5. Região de Interesse (ROI)")

    # --- Inicializar estado do ROI na sessão ---
    roi_defaults = {
        'gg_roi_mode': "Clique na imagem",
        'gg_roi_w_mm': 50.0,
        'gg_roi_h_mm': 50.0,
        'gg_roi_click_x': None,
        'gg_roi_click_y': None,
        'gg_roi_mask': None,
        'gg_roi_applied': False,
        'gg_roi_auto_thresh': 10,
    }
    ...
    # Dimensões da imagem
    h_px, w_px = film_aligned.shape
    h_mm = h_px * res_film_mm
    w_mm = w_px * res_film_mm

    roi_mode = st.radio(
        "Modo de ROI",
        ["Clique na imagem", "Automático (threshold de dose)"],
        index=0 if st.session_state['gg_roi_mode'] == "Clique na imagem" else 1,
        key="gg_roi_mode",
        horizontal=True
    )
    ...
    # ==========================================================
    # MODO CLIQUE NA IMAGEM
    # ==========================================================
    if roi_mode == "Clique na imagem":
        # ---- 1. CLICAR NA IMAGEM PARA DEFINIR CENTRO ----
        st.markdown("**1. Clique no centro da região que deseja analisar:**")

        if HAS_IMAGE_COORDS:
            # Normalizar imagem para uint8 RGB para o componente
            dose_norm = film_aligned.copy()
            dose_valid = dose_norm[np.isfinite(dose_norm)]
            if len(dose_valid) > 0 and dose_valid.max() > dose_valid.min():
                vmin, vmax = dose_valid.min(), dose_valid.max()
                dose_norm = (dose_norm - vmin) / (vmax - vmin)
            dose_norm = np.clip(dose_norm, 0, 1)
            dose_uint8 = (dose_norm * 255).astype(np.uint8)
            if dose_uint8.ndim == 2:
                img_pil = Image.fromarray(dose_uint8).convert('RGB')
            else:
                img_pil = Image.fromarray(dose_uint8)

            # Mostrar imagem clicável
            click_result = streamlit_image_coordinates(img_pil, key="gg_click_roi")

            if click_result is not None:
                st.session_state['gg_roi_click_x'] = int(click_result["x"])
                st.session_state['gg_roi_click_y'] = int(click_result["y"])
        else:
            st.error("⚠️ streamlit-image-coordinates não instalado. Use: `pip install streamlit-image-coordinates`")
            # Fallback: centro da imagem
            st.session_state['gg_roi_click_x'] = w_px // 2
            st.session_state['gg_roi_click_y'] = h_px // 2

        # Mostrar coordenadas do clique
        cx_click = st.session_state.get('gg_roi_click_x')
        cy_click = st.session_state.get('gg_roi_click_y')
        if cx_click is not None and cy_click is not None:
            st.success(f"📍 Centro selecionado: ({cx_click}, {cy_click}) px = ({cx_click*res_film_mm:.1f}, {cy_click*res_film_mm:.1f}) mm")

        # ---- 2. SLIDERS DE TAMANHO EM MM ----
        st.markdown("**2. Ajuste o tamanho do ROI:**")
        col_sz1, col_sz2 = st.columns(2)
        with col_sz1:
            roi_w = st.slider("Largura (mm)", 5.0, min(w_mm, 200.0), st.session_state['gg_roi_w_mm'], 5.0, key="gg_roi_w_mm_v13")
        with col_sz2:
            roi_h = st.slider("Altura (mm)", 5.0, min(h_mm, 200.0), st.session_state['gg_roi_h_mm'], 5.0, key="gg_roi_h_mm_v13")

        # ---- 3. CALCULAR RETÂNGULO A PARTIR DO CENTRO + TAMANHO ----
        if cx_click is not None and cy_click is not None:
            half_w_px = int((roi_w / res_film_mm) / 2)
            half_h_px = int((roi_h / res_film_mm) / 2)

            x1 = max(0, cx_click - half_w_px)
            y1 = max(0, cy_click - half_h_px)
            x2 = min(w_px, cx_click + half_w_px)
            y2 = min(h_px, cy_click + half_h_px)

            # Criar máscara
            roi_mask_click = np.zeros_like(film_aligned, dtype=bool)
            roi_mask_click[y1:y2, x1:x2] = True

            # ---- 4. PREVIEW VISUAL ----
            st.markdown("**3. Preview do ROI:**")
            fig_roi, ax_roi = plt.subplots(figsize=(9, 7))
            im_bg = ax_roi.imshow(film_aligned, cmap='jet', alpha=0.4)
            roi_disp = np.where(roi_mask_click, film_aligned, np.nan)
            im_roi = ax_roi.imshow(roi_disp, cmap='jet', alpha=0.9, vmin=film_aligned.min(), vmax=film_aligned.max())

            # Retângulo vermelho
            rect = plt.Rectangle((x1, y1), x2-x1, y2-y1, fill=False, edgecolor='red', linewidth=3)
            ax_roi.add_patch(rect)

            # Cruz no centro clicado
            ax_roi.plot(cx_click, cy_click, 'r+', markersize=15, mew=2)
            ax_roi.axvline(cx_click, color='red', linestyle=':', alpha=0.5, linewidth=1)
            ax_roi.axhline(cy_click, color='red', linestyle=':', alpha=0.5, linewidth=1)

            ax_roi.set_title(
                f"ROI: {x2-x1}×{y2-y1} px = {(x2-x1)*res_film_mm:.1f}×{(y2-y1)*res_film_mm:.1f} mm | "
                f"Centro: ({cx_click},{cy_click}) | {100*np.sum(roi_mask_click)/roi_mask_click.size:.1f}% da área"
            )
            ax_roi.axis('off')
            plt.colorbar(im_bg, ax=ax_roi, fraction=0.046)
            st.pyplot(fig_roi)

            # ---- 5. BOTÕES ----
            col_b1, col_b2 = st.columns(2)
            with col_b1:
                if st.button("✅ Aplicar este ROI", type="primary", key="gg_btn_apply_click"):
                    st.session_state['gg_roi_mask'] = roi_mask_click
                    st.session_state['gg_roi_applied'] = True
                    st.session_state['gg_roi_w_mm'] = roi_w
                    st.session_state['gg_roi_h_mm'] = roi_h
                    st.success(f"✅ ROI aplicado: {(x2-x1)*res_film_mm:.1f}×{(y2-y1)*res_film_mm:.1f} mm no centro ({cx_click},{cy_click}) px")
                    st.rerun()
            with col_b2:
                if st.button("🔄 Resetar (imagem inteira)", key="gg_btn_reset_click"):
                    st.session_state['gg_roi_mask'] = np.ones_like(film_aligned, dtype=bool)
                    st.session_state['gg_roi_applied'] = True
                    st.session_state['gg_roi_click_x'] = w_px // 2
                    st.session_state['gg_roi_click_y'] = h_px // 2
                    st.success("🔄 ROI resetado para imagem inteira")
                    st.rerun()

            roi_mask = st.session_state.get('gg_roi_mask', roi_mask_click)
        else:
            st.info("👆 Clique na imagem acima para definir o centro do ROI")
            roi_mask = st.session_state.get('gg_roi_mask')

    # ==========================================================
    # MODO AUTOMÁTICO
    # ==========================================================
    else:
        thresh_auto = st.slider(
            "Threshold de dose (% do máximo)", 1, 50,
            st.session_state['gg_roi_auto_thresh'], 1,
            key="gg_roi_auto_thresh_v13",
            help="TG-218 recomenda 10-20%."
        )

        roi_mask_auto, bbox_auto = detect_film_roi(film_aligned, threshold_percent=float(thresh_auto))
        st.session_state['gg_roi_mask'] = roi_mask_auto
        st.session_state['gg_roi_applied'] = True
        roi_mask = roi_mask_auto

        x, y, bw, bh = bbox_auto
        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(film_aligned, cmap='gray', alpha=0.3)
        roi_disp = np.where(roi_mask_auto, film_aligned, np.nan)
        im_roi = ax_roi.imshow(roi_disp, cmap='jet', alpha=0.7)
        rect = plt.Rectangle((x, y), bw, bh, fill=False, edgecolor='lime', linewidth=3)
        ax_roi.add_patch(rect)
        ax_roi.set_title(f"ROI Automática: {bw}×{bh} px = {bw*res_film_mm:.1f}×{bh*res_film_mm:.1f} mm")
        ax_roi.axis('off')
        plt.colorbar(im_roi, ax=ax_roi, fraction=0.046)
        st.pyplot(fig_roi)

    # ---- Info final ----
    if roi_mask is not None and roi_mask.size > 0:
        roi_px = int(np.sum(roi_mask))
        roi_pct = 100.0 * roi_px / roi_mask.size
        st.info(f"📊 ROI: **{roi_px} pixels** ({roi_pct:.1f}% da área) | {w_px}×{h_px} px total")

    # ============================================================
    # PARTE 6: PARÂMETROS GAMMA
    # ============================================================
    st.markdown("---")
    st.subheader("⚙️ 6. Parâmetros Gamma")

    col_p1, col_p2, col_p3 = st.columns(3)
    with col_p1:
        dd = st.selectbox("Dose Difference (%)", [1, 2, 3, 4, 5], index=2, key="gg_dd_v11")
    with col_p2:
        dta = st.selectbox("DTA (mm)", [1, 2, 3, 4, 5], index=2, key="gg_dta_v11")
    with col_p3:
        threshold_gamma = st.selectbox("Lower Dose Cutoff (%)", [5, 10, 15, 20, 25], index=1, key="gg_thresh_v11",
                                         help="TG-218 recomenda 10% (padrão) ou 20% para análise mais rigorosa.")

    global_norm = st.checkbox("Normalização Global (ref = % da dose máxima)", value=True, key="gg_gn_v11",
                               help="Se marcado, o critério de dose é % da dose máxima global. Se desmarcado, é % da dose local (mais rigoroso).")

    st.info(f"Critério: **{dd}% / {dta}mm** | Lower cutoff: **{threshold_gamma}%** | Norm: **{'Global' if global_norm else 'Local'}**")

    # ============================================================
    # PARTE 7: CALCULAR GAMMA
    # ============================================================
    st.markdown("---")
    if st.button("🚀 CALCULAR GAMMA", type="primary", key="gg_btn_calc"):
        with st.spinner("Calculando gamma... isso pode levar 1-3 minutos dependendo do tamanho da imagem"):
            try:
                gamma_res = gamma_analysis_optimized(
                    dose_film=film_aligned,
                    dose_tps=tps_shifted,
                    res_mm=res_film_mm,
                    roi_mask=roi_mask,
                    dose_percent=float(dd),
                    dta_mm=float(dta),
                    threshold_percent=float(threshold_gamma),
                    global_norm=global_norm,
                    max_gamma=2.0,
                )

                st.session_state['gg_result'] = gamma_res
                st.session_state['gg_roi_mask'] = roi_mask
                st.success(f"✅ Cálculo concluído! Passing Rate: **{gamma_res['passing_rate']:.1f}%**")
                st.rerun()
            except Exception as e:
    ...
    # PARTE 8: RESULTADOS
    # ============================================================
    if 'gg_result' in st.session_state and st.session_state['gg_result'] is not None:
        g = st.session_state['gg_result']
        ...
        col_m1.metric("Passing Rate", f"{g['passing_rate']:.1f}%", delta=f"{'PASS' if g['passing_rate'] >= 95 else 'FAIL'}" if g['passing_rate'] >= 90 else f"{'PASS' if g['passing_rate'] >= 90 else 'REVISAR'}")
        col_m2.metric("γ Médio", f"{g['gamma_mean']:.3f}")
        col_m3.metric("γ Mediano", f"{g['gamma_median']:.3f}")
        col_m4.metric("γ Máximo", f"{g['gamma_max']:.3f}")

        # 4 mapas: filme, TPS, gamma, overlay
        fig_r, ax_r = plt.subplots(2, 2, figsize=(14, 12))

        vmax_f = float(np.max(film_aligned))
        vmax_t = float(np.max(tps_shifted))
        vmax_both = max(vmax_f, vmax_t)

        im0 = ax_r[0,0].imshow(film_aligned, cmap='jet', vmin=0, vmax=vmax_both)
        ax_r[0,0].set_title(f"Filme ({best_flip}) — Max: {vmax_f:.2f} Gy")
        ax_r[0,0].axis('off')
        plt.colorbar(im0, ax=ax_r[0,0], fraction=0.046)

        im1 = ax_r[0,1].imshow(tps_shifted, cmap='jet', vmin=0, vmax=vmax_both)
        ax_r[0,1].set_title(f"TPS — Max: {vmax_t:.2f} Gy")
        ax_r[0,1].axis('off')
        plt.colorbar(im1, ax=ax_r[0,1], fraction=0.046)

        gamma_map = g['gamma_map']
        gamma_disp = np.where(g['valid_mask'], gamma_map, np.nan)
        im2 = ax_r[1,0].imshow(gamma_disp, cmap='RdYlGn_r', vmin=0, vmax=2)
        ax_r[1,0].set_title(f"Gamma Index — {g['passing_rate']:.1f}% pass | γ≤1")
        ax_r[1,0].axis('off')
        plt.colorbar(im2, ax=ax_r[1,0], fraction=0.046)

        # Overlay com contornos
        ax_r[1,1].imshow(film_aligned, cmap='gray', alpha=0.4)
        levels = [0.5 * vmax_f, 0.8 * vmax_f]
        ax_r[1,1].contour(film_aligned, levels=levels, colors='yellow', linewidths=2)
        ax_r[1,1].contour(tps_shifted, levels=levels, colors='cyan', linewidths=2, linestyles='--')
        ax_r[1,1].set_title("Overlay: Filme (amarelo) vs TPS (cyan)")
        ax_r[1,1].axis('off')
        ...
        if len(gv) > 0:
            ax_h.hist(gv, bins=50, color='steelblue', edgecolor='black', alpha=0.7)
            ax_h.axvline(1.0, color='red', linestyle='--', linewidth=2, label='γ = 1.0 (limite)')
            ax_h.axvline(np.mean(gv), color='green', linestyle='-', linewidth=2, label=f'γ médio = {np.mean(gv):.3f}')
            ax_h.set_xlabel("Gamma Index")
            ax_h.set_ylabel("Frequência (pixels)")
            ax_h.set_title(f"Histograma Gamma | Critério: {dd}% / {dta}mm | {len(gv)} pixels avaliados")
            ax_h.legend()
            ax_h.set_xlim(0, 2)
        st.pyplot(fig_h)
        ...
        resumo_df = pd.DataFrame([
            {"Parâmetro": "Critério Gamma", "Valor": f"{dd}% / {dta}mm"},
            {"Parâmetro": "Lower Dose Cutoff", "Valor": f"{threshold_gamma}%"},
            {"Parâmetro": "Normalização", "Valor": "Global" if global_norm else "Local"},
            {"Parâmetro": "Orientação do filme", "Valor": str(best_flip)},
            {"Parâmetro": "Translação X", "Valor": f"{tx:.1f} mm"},
            {"Parâmetro": "Translação Y", "Valor": f"{ty:.1f} mm"},
            {"Parâmetro": "Passing Rate", "Valor": f"{g['passing_rate']:.1f}%"},
            {"Parâmetro": "γ médio", "Valor": f"{g['gamma_mean']:.3f}"},
            {"Parâmetro": "γ mediano", "Valor": f"{g['gamma_median']:.3f}"},
            {"Parâmetro": "γ máximo", "Valor": f"{g['gamma_max']:.3f}"},
            {"Parâmetro": "Pixels avaliados", "Valor": f"{g['n_evaluated']} / {g['n_total']}"},
            {"Parâmetro": "Resolução", "Valor": f"{res_film_mm:.4f} mm/px"},
        ])
        st.dataframe(resumo_df, use_container_width=True, hide_index=True)

        # Download do resumo como CSV
        csv_resumo = resumo_df.to_csv(index=False)
        st.download_button("📥 Download Resumo (CSV)", csv_resumo, "gamma_resumo.csv", "text/csv")

        # Critério de aprovação TG-218
        st.subheader("📋 Critério de Aprovação (TG-218)")
        if g['passing_rate'] >= 95:
            st.success(f"✅ **APROVADO** — Passing rate {g['passing_rate']:.1f}% ≥ 95% (critério TG-218 estrito)")
        elif g['passing_rate'] >= 90:
            st.warning(f"⚠️ **MARGINAL** — Passing rate {g['passing_rate']:.1f}% entre 90-95%. Revisar configuração.")
        else:
            st.error(f"❌ **REPROVADO** — Passing rate {g['passing_rate']:.1f}% < 90%. Verificar filme e registro.")


# ==================== COMPARAÇÃO DE ISODOSE ====================
# Overlay de isodoses filme vs TPS + % de coincidência

elif metodologia == "Comparação de Isodose":
    st.header("🔄 Comparação de Isodose — Filme vs TPS")
    st.info("Sobreposição de curvas de isodose do filme radiocrômico com o TPS. Analise a coincidência e calcule o percentual de sobreposição.")

    # --- Upload do filme irradiado ---
    st.subheader("1. Filme Irradiado")
    up_irr = st.file_uploader("Upload do filme irradiado (TIFF/PNG)", type=['tif','tiff','png','jpg'], key="iso_film")

    # --- Upload do TPS ---
    st.subheader("2. Dose do TPS")
    up_tps = st.file_uploader("Upload do arquivo TPS (.ALL, .csv, .dcm)", type=['all','csv','txt','dcm','png','jpg','tif'], key="iso_tps")

    # --- Upload do 0 Gy ---
    st.subheader("3. Filme de Referência (0 Gy)")
    up_0gy = st.file_uploader("Upload do filme 0 Gy / background", type=['tif','tiff','png','jpg'], key="iso_0gy")

    # --- Curva de calibração ---
    st.subheader("4. Curva de Calibração")
    curva_up = st.file_uploader("Upload da curva de calibração (.json)", type=['json'], key="iso_curva")

    # --- Parâmetros ---
    st.subheader("5. Parâmetros")
    col1, col2 = st.columns(2)
    with col1:
        dpi = st.number_input("DPI do scanner", 72, 2400, 150, key="iso_dpi")
        res_mm = 25.4 / dpi
        presc_dose = st.number_input("Dose de prescrição (Gy)", 0.1, 100.0, 10.0, 0.1, key="iso_presc",
                                       help="Dose de prescrição do plano. Ex: 10 Gy. As isodoses serão 50%, 75%, 100%, 125%, 150% deste valor.")
    with col2:
        isodoses = st.multiselect("Isodoses (% da prescrição)", [50, 75, 100, 125, 150], default=[50, 100, 150], key="iso_levels")
        tol_mm = st.number_input("Tolerância (mm)", 0.5, 10.0, 3.0, 0.5, key="iso_tol")

    if up_irr and up_tps and up_0gy and curva_up:
        st.success("✅ Todos os arquivos carregados. Pronto para calcular.")
        if st.button("🚀 CALCULAR COINCIDÊNCIA", type="primary", key="iso_calc"):
            progress_bar = st.progress(0, text="Iniciando processamento...")
            
            try:
                from isodose_engine import compare_isodose
                from gamma_engine import resize_to_shape, register_with_flip, apply_shift
                from skimage.measure import find_contours
                
                # 1. Carregar imagens
                progress_bar.progress(5, text="1/6 - Carregando imagens...")
                img_irr, _ = carregar_imagem_preservando_bits(up_irr)
                img_0gy, _ = carregar_imagem_preservando_bits(up_0gy)
                
                # 2. Processar filme
                progress_bar.progress(20, text="2/6 - Detectando e cortando filmes...")
                filme_irr = cortar_filme_unico(img_irr)
                filme_0gy = cortar_filme_unico(img_0gy)
                if filme_irr is None or filme_0gy is None:
                    st.error("❌ Nao foi possivel detectar o filme na imagem.")
                    st.stop()
                
                filme_irr_sf, _, _ = remover_fundo_branco(filme_irr)
                filme_0gy_sf, _, _ = remover_fundo_branco(filme_0gy)
                
                progress_bar.progress(30, text="3/6 - Calculando dose do filme...")
                red_0gy = canal_vermelho(filme_0gy_sf)
                pv0 = float(np.mean(red_0gy))
                red_irr = canal_vermelho(filme_irr_sf)
                adc_media = float(np.mean(red_irr))
                adc_aumenta = adc_media > pv0
                
                # 3. Curva de calibração
                curva_raw = json.load(curva_up)
                curva = curva_raw.get('curva', curva_raw)
                curva_dict = {}
                for k in ['a', 'b', 'c', 'K1', 'K2']:
                    if k in curva:
                        curva_dict[k] = float(curva[k])
                
                # 4. Calcular dose do filme
                dose_film = calcular_mapa_dose(filme_irr_sf, pv0, curva_dict, adc_aumenta)
                dose_film = np.where(np.isfinite(dose_film), dose_film, 0)
                dose_film = np.clip(dose_film, 0, None)
                
                # 5. Ler TPS
                progress_bar.progress(50, text="4/6 - Lendo TPS + registro...")
                up_tps.seek(0)
                tps_dist = read_tps(up_tps.read(), filename=up_tps.name)
                dose_tps_raw = tps_dist.dose
                dose_tps = resize_to_shape(dose_tps_raw, dose_film.shape)
                
                # 6. Registro
                film_aligned, best_flip, tx, ty, score = register_with_flip(dose_film, dose_tps, res_mm)
                tps_shifted = apply_shift(dose_tps, tx, ty, res_mm)
                
                # 7. Comparação de isodose com find_contours
                progress_bar.progress(75, text="5/6 - Extraindo curvas de isodose...")
                iso_levels = [int(x) for x in isodoses]
                iso_result = compare_isodose(
                    dose_film=film_aligned,
                    dose_tps=tps_shifted,
                    prescription_dose=presc_dose,
                    levels=iso_levels,
                    tolerance_mm=tol_mm,
                    res_mm=res_mm,
                )
                ...
                # 8. Resultados
                st.markdown("---")
                st.header("📊 Resultados da Comparação de Isodose")
                st.write(f"Dose de prescrição: **{presc_dose:.2f} Gy** | Registro: flip={best_flip}")
                
                # Tabela
                df = pd.DataFrame([
                    {
                        'Isodose (%)': f"{r['level']}%",
                        'Dose (Gy)': f"{r['dose_value']:.2f}",
                        'Coincidência (%)': f"{r['coincidence']:.1f}%",
                        'Dist. Média (mm)': f"{r['mean_distance_mm']:.2f}",
                        'Contornos Filme': r['n_contours_film'],
                        'Contornos TPS': r['n_contours_tps'],
                    }
                    for r in iso_result['results']
                ])
                st.dataframe(df, use_container_width=True, hide_index=True)
                
                # Métricas
                cols = st.columns(len(iso_result['results']))
                for i, r in enumerate(iso_result['results']):
                    status = "✅" if r['coincidence'] >= 90 else "⚠️" if r['coincidence'] >= 80 else "❌"
                    cols[i].metric(f"{r['level']}%", f"{r['coincidence']:.1f}%", f"{status}")
                
                # Visualização: contornos reais com find_contours
                st.subheader("Curvas de Isodose — Filme vs TPS")
                fig, axes = plt.subplots(1, 2, figsize=(14, 6))
                
                # Cores para cada isodose
                iso_colors = {50: '#1e3a5f', 75: '#2563eb', 100: '#10b981', 125: '#f59e0b', 150: '#dc2626'}
                
                # Filme
                axes[0].imshow(film_aligned, cmap='gray', alpha=0.4)
                for pct, contours in iso_result['contours_film'].items():
                    color = iso_colors.get(pct, '#666')
                    for c in contours:
                        axes[0].plot(c[:, 1], c[:, 0], color=color, linewidth=2, label=f'{pct}%' if c is contours[0] else '')
                axes[0].set_title(f"Filme ({best_flip})")
                axes[0].axis('off')
                axes[0].legend(loc='upper right', title="Isodose")
                
                # TPS
                axes[1].imshow(tps_shifted, cmap='gray', alpha=0.4)
                for pct, contours in iso_result['contours_tps'].items():
                    color = iso_colors.get(pct, '#666')
                    for c in contours:
                        axes[1].plot(c[:, 1], c[:, 0], color=color, linewidth=2, label=f'{pct}%' if c is contours[0] else '')
                axes[1].set_title("TPS Alinhado")
                axes[1].axis('off')
                axes[1].legend(loc='upper right', title="Isodose")
                ...
                # Download CSV
                csv = df.to_csv(index=False)
                st.download_button("📥 Download Resultados (CSV)", csv, "comparacao_isodose.csv", "text/csv")
        ...
        st.warning("⚠️ Preencha todos os campos acima para prosseguir.")

# ============================================================
# SEÇÃO: GERAR RELATÓRIO
# ============================================================
st.markdown("---")
st.header("📄 Gerar Relatório")

st.info("Gere um relatorio profissional em PDF com todas as analises realizadas nesta sessao.")

# Verificar quais analises foram feitas
analises_feitas = []
if 'curva_calibracao' in st.session_state:
    analises_feitas.append("OK Calibracao")
if 'mapa_dose_gerado' in st.session_state:
    analises_feitas.append("OK Mapa de Dose")
if 'gamma_result' in st.session_state:
    analises_feitas.append("OK Analise Gamma")

if analises_feitas:
    st.write("**Analises disponiveis no relatorio:**")
    for a in analises_feitas:
        st.write(a)
    
    if st.button("GERAR RELATORIO PDF", type="primary", key="btn_relatorio"):
            with st.spinner("Gerando relatório profissional..."):
                try:
                    # Coletar dados da sessão
                    analises = {
                        'calibracao': {'feita': 'curva_calibracao' in st.session_state},
                        'dose_map': {'feita': 'mapa_dose_gerado' in st.session_state},
                        'isodose': {'feita': 'mapa_dose_gerado' in st.session_state},
                        'gamma': {'feita': 'gamma_result' in st.session_state},
                        'iso_compare': {'feita': False},
                    }
                    
                    if 'curva_calibracao' in st.session_state:
                        cc = st.session_state['curva_calibracao']
                        analises['calibracao']['curva'] = cc
                        analises['calibracao']['r2'] = cc.get('r2', 0.999)
                    
                    if 'gamma_result' in st.session_state:
                        gg = st.session_state['gamma_result']
                        analises['gamma'].update({
                            'dd': st.session_state.get('gg_dd_v11', 3),
                            'dta': st.session_state.get('gg_dta_v11', 3),
                            'passing_rate': gg.get('passing_rate', 0),
                            'gamma_mean': gg.get('gamma_mean', 0),
                            'gamma_max': gg.get('gamma_max', 0),
                        })
                    
                    pdf_path = gerar_relatorio_pdf(analises)
                    ...
                    st.download_button(
                        ...
                        data=pdf_bytes,
                        file_name=f"Chromis_Relatorio_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
        ...
        st.warning("⚠️ Nenhuma analise foi realizada nesta sessao. Execute pelo menos uma analise para gerar o relatorio.")
...
# ============================================================
st.markdown("---")
st.caption("Chromis v3.2 | Dosimetria com Filmes Radiocrômicos EBT3/EBT4 | Análise Gamma TG-218")
