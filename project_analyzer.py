...
# ========== FUNCAO RELATORIO (com acentos, logo centralizada, graficos reais) ==========
...
def gerar_relatorio_pdf(analises, info_paciente=None, output_path='/tmp/chromis_relatorio.pdf'):
    ...
    with PdfPages(output_path) as pdf:
        ...
        # Logo centralizada no topo (sem sobreposicao)
        try:
            logo = PILImage.open('Chromis_logo_principal.png')
            logo_w = 2.5  # maior
            aspect = logo.height / logo.width
            logo_h = logo_w * aspect
            # Centro: (8.27 - logo_w) / 2 = 2.885
            ax.imshow(logo, extent=[2.885, 2.885 + logo_w, 9.5, 9.5 + logo_h], aspect='auto', zorder=5)
        except:
            pass

        # Titulo abaixo da logo (sem sobreposicao)
        ax.text(4.135, 8.8, 'RELATORIO DE DOSIMETRIA',
                fontsize=24, fontweight='bold', ha='center', color='#1e3a5f', zorder=10)
        ax.text(4.135, 8.3, 'com Filmes Radiocromicos EBT3/EBT4',
                fontsize=14, ha='center', color='#64748b', zorder=10)

        # Caixa de informacoes
        rect = plt.Rectangle((1, 5.0), 6.27, 2.5, fill=True,
        ...
        y_anal = 3.4
        for nome, feita in [
            ('Calibração', analises.get('calibracao', {}).get('feita', False)),
            ('Mapa de Dose 2D', analises.get('dose_map', {}).get('feita', False)),
            ('Mapa de Isodoses', analises.get('isodose', {}).get('feita', False)),
            ('Análise Gamma', analises.get('gamma', {}).get('feita', False)),
            ('Comparação de Isodose', analises.get('iso_compare', {}).get('feita', False)),
        ]:
        ...
        # Rodape
        ax.text(4.135, 0.8, 'Desenvolvido por MACIEL, J. O. | Chromis v3.2',
        ...
        if analises.get('calibracao', {}).get('feita'):
            ...
            # Titulo
            fig.suptitle('1. Curva de Calibração', fontsize=16, fontweight='bold',
                         color='#1e3a5f', y=0.98)
            ...
            try:
                ...
                if 'a' in curva and 'b' in curva and 'c' in curva:
                    ...
                    ax1.plot(nod_fit, dose_fit, 'r-', linewidth=2, label=f'Ajuste (R²={r2:.4f})')
                elif 'K1' in curva and 'K2' in curva:
                ...
                ax1.text(0.5, 0.5, 'Curva de calibração\n(Grafico gerado na sessão)',
                         transform=ax1.transAxes, ha='center', va='center',
                         fontsize=12, color='#9ca3af')

            ax1.set_xlabel('NOD (Net Optical Density)', fontsize=11)
            ax1.set_ylabel('Dose (Gy)', fontsize=11)
            ax1.set_title(f'Curva de Calibração — R² = {r2:.4f}', fontsize=13)
            ax1.legend()
            ...
            params.append(['R²', f'{r2:.4f}'])

            if params:
                table = ax2.table(cellText=params, colLabels=['Parâmetro', 'Valor'],
        ...
        if analises.get('dose_map', {}).get('feita'):
            ...
            fig.suptitle('2. Mapa de Dose 2D', fontsize=16, fontweight='bold',
                         color='#1e3a5f', y=0.98)
            ...
            ax1 = axes[0]
            if 'dose_map_path' in analises['dose_map']:
                try:
                    img = PILImage.open(analises['dose_map']['dose_map_path'])
                    ax1.imshow(img)
                except:
                    ax1.text(0.5, 0.5, 'Mapa de Dose\n(Filme)',
                             transform=ax1.transAxes, ha='center', va='center',
                             fontsize=12, color='#9ca3af')
            else:
                ax1.text(0.5, 0.5, 'Mapa de Dose\n(Filme)',
            ...
            ax1.set_title(f'Filme — Max: {max_dose:.2f} Gy', fontsize=11)
            ax1.axis('off')
            ...
            infos = [
                ['Informação', 'Valor'],
                ['Dose Máxima', f'{max_dose:.2f} Gy'],
                ['Tipo de Filme', 'EBT3/EBT4'],
            ]
            table = ax2.table(cellText=infos[1:], colLabels=infos[0],
                               loc='center', cellLoc='left')
            table.auto_set_font_size(False)
        ...
        if analises.get('gamma', {}).get('feita'):
            ...
            fig.suptitle('3. Análise Gamma', fontsize=16, fontweight='bold',
                         color='#1e3a5f', y=0.98)
            ...
            metrics = [
                ['Métrica', 'Valor'],
                ['Critério', f"{gg.get('dd', '?')}% / {gg.get('dta', '?')}mm"],
                ['Passing Rate', f"{gg.get('passing_rate', 0):.1f}%"],
                ['Gama Médio', f"{gg.get('gamma_mean', 0):.3f}"],
                ['Gama Máximo', f"{gg.get('gamma_max', 0):.3f}"],
            ]
            table = ax1.table(cellText=metrics[1:], colLabels=metrics[0],
                               loc='center', cellLoc='left')
            table.auto_set_font_size(False)
            table.set_fontsize(10)
            table.scale(1, 2)

            # Placeholder para graficos gamma
            for ax, title in [(axes[0, 1], 'Mapa Gamma'), (axes[1, 0], 'Histograma'), (axes[1, 1], 'Overlay')]:
        ...
        ax.text(4.135, 10.5, 'RESUMO E CONCLUSAO', fontsize=18, fontweight='bold',
        ...
        iso_ok = all(r.get('coincidence', 0) >= 80 for r in
                        analises.get('iso_compare', {}).get('results', []))
        aprovado = gamma_ok and iso_ok

        color = '#059669' if aprovado else '#dc2626'
        texto = 'APROVADO' if aprovado else 'REPROVADO'
        ax.text(4.135, 8.5, f'Status: {texto}', fontsize=22, fontweight='bold',
                ha='center', color=color)

        # Assinaturas
        ax.plot([1, 3.5], [3.0, 3.0], color='#374151', linewidth=1)
        ax.text(1, 2.6, 'Assinatura do Fisico Médico', fontsize=10, color='#6b7280')
