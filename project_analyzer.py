...
def gerar_relatorio_pdf(analises, info_paciente=None, output_path='/tmp/chromis_relatorio.pdf'):
    """Gera relatorio PDF usando matplotlib (ja instalado)."""
    ...
    with PdfPages(output_path) as pdf:
        ...
        ax.text(4.1, 9.5, 'RELATORIO DE DOSIMETRIA', fontsize=22, fontweight='bold', ha='center', color='#1e3a5f')
        ax.text(4.1, 9.0, 'com Filmes Radiocromicos EBT3/EBT4', fontsize=14, ha='center', color='#64748b')

        rect = plt.Rectangle((1, 5.5), 6.27, 2.5, fill=True, facecolor='#f1f5f9', edgecolor='#1e3a5f', linewidth=2)
        ax.add_patch(rect)
        ...
        ax.text(4.1, 1.0, 'Desenvolvido por MACIEL, J. O. | Chromis v3.2', fontsize=8, ha='center', color='#9ca3af', style='italic')
        pdf.savefig(fig, dpi=150, bbox_inches='tight', facecolor='white')
        plt.close(fig)

        # PAGINAS CONDICIONAIS
        analises_map = [
            ('calibracao', '1. Curva de Calibracao', 'Calib.'),
            ('dose_map', '2. Mapa de Dose 2D', 'Dose'),
            ('gamma', '3. Analise Gamma', 'Gamma'),
        ]

        for key, title, label in analises_map:
            if analises.get(key, {}).get('feita'):
                fig, ax = plt.subplots(figsize=(8.27, 11.69))
                ax.set_xlim(0, 8.27)
                ax.set_ylim(0, 11.69)
                ax.axis('off')
                fig.patch.set_facecolor('white')
                ax.text(0.5, 10.8, title, fontsize=16, fontweight='bold', color='#1e3a5f')
                ax.plot([0.5, 7.77], [10.5, 10.5], color='#1e3a5f', linewidth=2)

                data = analises[key]
                y = 9.5
                if key == 'calibracao':
                    ax.text(0.5, y, f'Tipo: {data.get("curva", {}).get("tipo_fitting", "N/A")}', fontsize=11); y -= 0.4
                    ax.text(0.5, y, f'R2: {data.get("r2", 0):.4f}', fontsize=11); y -= 0.4
                elif key == 'dose_map':
                    ax.text(0.5, y, f'Dose maxima: {data.get("max_dose", 0):.2f} Gy', fontsize=11); y -= 0.4
                elif key == 'gamma':
                    ax.text(0.5, y, f'Criterio: {data.get("dd", "?")}% / {data.get("dta", "?")}mm', fontsize=11); y -= 0.4
                    passing = data.get('passing_rate', 0)
                    ax.text(0.5, y, f'Passing Rate: {passing:.1f}%', fontsize=11, color='#059669' if passing >= 90 else '#dc2626', fontweight='bold'); y -= 0.4
                    ax.text(0.5, y, f'Gamma medio: {data.get("gamma_mean", 0):.3f}', fontsize=11); y -= 0.4
                    ax.text(0.5, y, f'Gamma maximo: {data.get("gamma_max", 0):.3f}', fontsize=11)

                pdf.savefig(fig, dpi=150, bbox_inches='tight', facecolor='white')
                plt.close(fig)

        # COMPARACAO DE ISODOSE
        if analises.get('iso_compare', {}).get('feita'):
            ...
            ax.axis('off')
            fig.patch.set_facecolor('white')
            ax.text(0.5, 10.8, '4. Comparacao de Isodose', fontsize=16, fontweight='bold', color='#1e3a5f')
            ax.plot([0.5, 7.77], [10.5, 10.5], color='#1e3a5f', linewidth=2)

            results = analises['iso_compare'].get('results', [])
            y = 9.5
            ax.text(0.5, y, 'Isodose    Dose(Gy)    Coincid.    Dist.mm', fontsize=10, fontweight='bold', color='#1e3a5f'); y -= 0.4
            for r in results:
                ax.text(0.5, y, f'{r.get("level", 0)}%', fontsize=9)
                ax.text(2.0, y, f'{r.get("dose_value", 0):.2f}', fontsize=9)
                coin = r.get('coincidence', 0)
                ax.text(3.5, y, f'{coin:.1f}%', fontsize=9, color='#059669' if coin >= 90 else '#dc2626')
                ax.text(5.5, y, f'{r.get("mean_distance_mm", 0):.2f}', fontsize=9)
                y -= 0.35
        ...
        ax.axis('off')
        fig.patch.set_facecolor('white')
        ax.text(0.5, 10.8, 'RESUMO E CONCLUSAO', fontsize=16, fontweight='bold', color='#1e3a5f')
        ax.plot([0.5, 7.77], [10.5, 10.5], color='#1e3a5f', linewidth=2)
        ...
        text = 'APROVADO' if aprovado else 'REPROVADO'
        ax.text(0.5, 9.5, f'Status: {text}', fontsize=18, fontweight='bold', color=color)
