...
metodologia = st.radio("Qual a metodologia?", ["Um unico filme", "Varios filmes", "Analise Gamma"], horizontal=True)
st.markdown("---")
...
elif metodologia == "Varios filmes":
...
# ==================== ANÁLISE GAMMA (REESCRITA v11) ====================
    ...
    with st.expander("⚙️ Ajuste Manual de Registro (Flip + Translação)"):
        ...
        with col_m1:
            tx = st.slider("Translação X (mm)", -100.0, 100.0, float(tx), 0.5, key="gg_tx_manual")
            flip_h = st.checkbox("Flip Horizontal", value=(best_flip in ['h', 'hv']), key="gg_fh_manual")
        with col_m2:
            ty = st.slider("Translação Y (mm)", -100.0, 100.0, float(ty), 0.5, key="gg_ty_manual")
    ...
    # ============================================================
    # PARTE 5: ROI (REGIÃO DE INTERESSE) — REESCRITO v12
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 5. Região de Interesse (ROI)")
    st.info("Defina a área onde o gamma será calculado. Baseado no padrão Dosepy/OmniPro: tamanho e posição em **milímetros** (mm).")

    # --- Inicializar estado do ROI na sessão ---
    roi_defaults = {
        'gg_roi_mode_v12': "Automático (threshold de dose)",
        'gg_roi_w_mm': 60.0,      # largura ROI em mm
        'gg_roi_h_mm': 60.0,      # altura ROI em mm
        'gg_roi_cx_mm': 0.0,      # offset centro X em mm (0 = centro da imagem)
        'gg_roi_cy_mm': 0.0,      # offset centro Y em mm (0 = centro da imagem)
        'gg_roi_mask': None,       # máscara binária aplicada
        'gg_roi_applied': False,   # flag se ROI foi aplicado
        'gg_roi_auto_thresh': 10,  # threshold do modo automático
    }
    for k, v in roi_defaults.items():
        if k not in st.session_state:
            st.session_state[k] = v

    # Dimensões da imagem em pixels e mm
    ...
    roi_mode = st.radio(
        "Modo de ROI",
        ["Automático (threshold de dose)", "Manual (mm + preview visual)"],
        index=0 if st.session_state['gg_roi_mode_v12'] == "Automático (threshold de dose)" else 1,
        key="gg_roi_mode_v12"
    )

    roi_mask = None  # será definido abaixo

    # ==========================================================
    # MODO AUTOMÁTICO
    # ==========================================================
    if roi_mode == "Automático (threshold de dose)":
        st.session_state['gg_roi_mode_v12'] = "Automático (threshold de dose)"

        thresh_auto = st.slider(
            "Threshold de dose (% do máx)", 1, 50,
            st.session_state['gg_roi_auto_thresh'], 1,
            key="gg_roi_auto_thresh",
            help="Pixels com dose abaixo deste percentual do máximo são excluídos. TG-218 recomenda 10-20%."
        )

        # Recalcular ROI automática
        roi_mask_auto, bbox_auto = detect_film_roi(film_aligned, threshold_percent=float(thresh_auto))

        # Salvar automaticamente no modo automático (não precisa de botão)
        st.session_state['gg_roi_mask'] = roi_mask_auto
        st.session_state['gg_roi_applied'] = True
        roi_mask = roi_mask_auto

        # Preview da ROI automática
        x, y, bw, bh = bbox_auto
        ...
        ax_roi.set_title(f"ROI Automática: {bw}×{bh} px | {bw*res_film_mm:.1f}×{bh*res_film_mm:.1f} mm | {100*np.sum(roi_mask_auto)/roi_mask_auto.size:.1f}% da área")
        ax_roi.axis('off')
        plt.colorbar(im_roi, ax=ax_roi, fraction=0.046)
        st.pyplot(fig_roi)

    # ==========================================================
    # MODO MANUAL (mm + preview)
    # ==========================================================
    else:
        st.session_state['gg_roi_mode_v12'] = "Manual (mm + preview visual)"

        st.info(f"Dimensão da imagem: **{w_px}×{h_px} px** = **{w_mm:.1f}×{h_mm:.1f} mm** | Resolução: {res_film_mm:.3f} mm/px")

        # ---- Sliders de tamanho e posição em mm ----
        col_s1, col_s2 = st.columns(2)
        with col_s1:
            roi_w = st.slider("Largura ROI (mm)", 1.0, min(w_mm, 300.0), st.session_state['gg_roi_w_mm'], 1.0, key="gg_roi_w_mm")
            roi_h = st.slider("Altura ROI (mm)", 1.0, min(h_mm, 300.0), st.session_state['gg_roi_h_mm'], 1.0, key="gg_roi_h_mm")
        with col_s2:
            roi_cx = st.slider("Offset Centro X (mm)", -w_mm/2, w_mm/2, st.session_state['gg_roi_cx_mm'], 0.5, key="gg_roi_cx_mm",
                                help="0 = centro da imagem. Positivo = direita, Negativo = esquerda")
            roi_cy = st.slider("Offset Centro Y (mm)", -h_mm/2, h_mm/2, st.session_state['gg_roi_cy_mm'], 0.5, key="gg_roi_cy_mm",
                                help="0 = centro da imagem. Positivo = cima, Negativo = baixo")

        # ---- Converter mm → pixels ----
        roi_w_px = int(roi_w / res_film_mm)
        roi_h_px = int(roi_h / res_film_mm)
        center_x_px = w_px // 2 + int(roi_cx / res_film_mm)
        center_y_px = h_px // 2 + int(roi_cy / res_film_mm)

        # Garantir limites
        x1 = max(0, center_x_px - roi_w_px // 2)
        y1 = max(0, center_y_px - roi_h_px // 2)
        x2 = min(w_px, x1 + roi_w_px)
        y2 = min(h_px, y1 + roi_h_px)

        # Ajustar se ficou menor que o esperado (bordas)
        if x2 - x1 < roi_w_px and x1 > 0:
            x1 = max(0, x2 - roi_w_px)
        if y2 - y1 < roi_h_px and y1 > 0:
            y1 = max(0, y2 - roi_h_px)
        x1 = max(0, x1)
        y1 = max(0, y1)
        x2 = min(w_px, x1 + roi_w_px)
        y2 = min(h_px, y1 + roi_h_px)

        # ---- Criar máscara preview ----
        roi_mask_preview = np.zeros_like(film_aligned, dtype=bool)
        roi_mask_preview[y1:y2, x1:x2] = True

        # ---- Preview visual com retângulo ----
        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(film_aligned, cmap='gray', alpha=0.3)
        roi_disp = np.where(roi_mask_preview, film_aligned, np.nan)
        im_roi = ax_roi.imshow(roi_disp, cmap='jet', alpha=0.7)
        ...
        # Cruz no centro da imagem
        ax_roi.axvline(w_px//2, color='white', linestyle='--', alpha=0.3, linewidth=1)
        ax_roi.axhline(h_px//2, color='white', linestyle='--', alpha=0.3, linewidth=1)

        ax_roi.set_title(
            f"ROI Preview: {x2-x1}×{y2-y1} px = {(x2-x1)*res_film_mm:.1f}×{(y2-y1)*res_film_mm:.1f} mm | "
            f"Centro: ({center_x_px},{center_y_px}) px | {100*np.sum(roi_mask_preview)/roi_mask_preview.size:.1f}% da área"
        )
        ax_roi.axis('off')
        plt.colorbar(im_roi, ax=ax_roi, fraction=0.046)
        st.pyplot(fig_roi)

        # ---- Botão Aplicar ROI (persistência) ----
        col_btn1, col_btn2 = st.columns(2)
        with col_btn1:
            if st.button("✅ Aplicar ROI", type="primary", key="gg_btn_apply_roi"):
                st.session_state['gg_roi_mask'] = roi_mask_preview
                st.session_state['gg_roi_applied'] = True
                st.session_state['gg_roi_w_mm'] = roi_w
                st.session_state['gg_roi_h_mm'] = roi_h
                st.session_state['gg_roi_cx_mm'] = roi_cx
                st.session_state['gg_roi_cy_mm'] = roi_cy
                st.success(f"✅ ROI aplicado: {(x2-x1)*res_film_mm:.1f}×{(y2-y1)*res_film_mm:.1f} mm em ({roi_cx:+.1f}, {roi_cy:+.1f}) mm do centro")
                st.rerun()

        with col_btn2:
            if st.button("🔄 Resetar ROI (imagem inteira)", key="gg_btn_reset_roi"):
                st.session_state['gg_roi_mask'] = np.ones_like(film_aligned, dtype=bool)
                st.session_state['gg_roi_applied'] = True
                st.session_state['gg_roi_w_mm'] = w_mm
                st.session_state['gg_roi_h_mm'] = h_mm
                st.session_state['gg_roi_cx_mm'] = 0.0
                st.session_state['gg_roi_cy_mm'] = 0.0
                st.success("🔄 ROI resetado para imagem inteira")
                st.rerun()

        # ---- Usar máscara salva ou preview ----
        roi_mask = st.session_state.get('gg_roi_mask', roi_mask_preview)

        # Indicador visual se ROI foi aplicado
        if st.session_state.get('gg_roi_applied', False):
            st.success("✅ ROI aplicado e salvo — será usado no cálculo gamma")
        else:
            st.warning("⚠️ ROI preview ativo mas NÃO aplicado. Clique 'Aplicar ROI' para confirmar.")

    # ---- Info final da ROI ----
    if roi_mask is not None and roi_mask.size > 0:
        roi_px = int(np.sum(roi_mask))
        roi_pct = 100.0 * roi_px / roi_mask.size
        st.info(f"📊 ROI final: **{roi_px} pixels** ({roi_pct:.1f}% da área) | Shape: {w_px}×{h_px} px | Resolução: {res_film_mm:.3f} mm/px")
