...
from gamma_engine import resize_to_shape, apply_shift, register_with_flip, detect_film_roi, gamma_analysis
...
if metodologia == "Um unico filme":
    ...
    metodologia = st.radio("", ["Um unico filme", "Varios filmes", "Analise Gamma"], index=0)
...
# ==================== ANÁLISE GAMMA ====================
    ...
    # ============================================================
    # PARTE 1: OBTER MAPA DE DOSE DO FILME
    # ============================================================
    ...
    if 'mapa_dose_filme' in st.session_state:
        ...
        if usar_existente:
            ...
            res_film_mm = st.session_state.get('res_film_mm', 0.35)
            st.success(f"✅ Filme: {dose_film.shape[1]}×{dose_film.shape[0]} px | {res_film_mm:.3f} mm/px")

    # Opção B: gerar na aba Gamma (upload de background + irradiado + curva)
    if dose_film is None:
        st.markdown("**Gerar mapa de dose na aba Gamma:**")

        col1, col2 = st.columns(2)
        with col1:
            up_irr = st.file_uploader("Filme IRRADIADO", type=['tif','tiff','png','jpg'], key="gg_irr")
        with col2:
            up_0gy = st.file_uploader("Filme 0 Gy (BACKGROUND)", type=['tif','tiff','png','jpg'], key="gg_0gy")

        curva_up = st.file_uploader("Curva de calibração (.json)", type=['json'], key="gg_curva")
        if curva_up:
            curva = json.load(curva_up)
        elif 'curva_calibracao' in st.session_state:
            curva = st.session_state['curva_calibracao']
            st.info("Usando curva da sessão")
        else:
            curva = None

        if up_irr and up_0gy and curva:
            with st.spinner("Processando filme..."):
                img_irr = np.array(Image.open(up_irr)).astype(float)
                img_0gy = np.array(Image.open(up_0gy)).astype(float)

                c_irr = img_irr[:,:,0] if img_irr.ndim == 3 else img_irr
                c_0gy = img_0gy[:,:,0] if img_0gy.ndim == 3 else img_0gy

                # Detectar direção do scanner
                bordas = np.mean([c_irr[0,:], c_irr[-1,:], c_irr[:,0], c_irr[:,-1]])
                centro = np.mean(c_irr[c_irr.shape[0]//3:2*c_irr.shape[0]//3, c_irr.shape[1]//3:2*c_irr.shape[1]//3])
                adc_aumenta = bordas > centro
                ...
                if tipo == 'power':
                    ...
                    dose_film = (k1 + k2 * nod) / (1 + curva.get('K3', 0) * nod)
                ...
                st.success(f"✅ Mapa gerado: {dose_film.shape[1]}×{dose_film.shape[0]} px | Max: {dose_film.max():.2f} Gy")
        else:
            st.warning("Envie: filme irradiado + filme 0 Gy + curva de calibração")
            st.stop()

    # Resolução do filme
    if res_film_mm is None:
        res_film_mm = st.number_input("Resolução do filme (mm/px)", 0.01, 10.0, 0.35, 0.01, format="%.3f", key="gg_res")
        dpi = st.number_input("Ou DPI do scanner", 10, 2400, 72, key="gg_dpi")
        if dpi > 0:
            res_film_mm = 25.4 / dpi

    st.info(f"📐 Resolução do filme: **{res_film_mm:.3f} mm/px**")
    ...
    arq_tps = st.file_uploader("Arquivo TPS (.ALL, .csv, .dcm, .png)", type=['all','csv','txt','dcm','png','jpg','tif'], key="gg_tps")

    if not arq_tps:
        st.info("Envie o arquivo de dose do TPS")
        st.stop()

    with st.spinner("Lendo TPS..."):
        try:
            tps_dist = read_tps(arq_tps.read(), filename=arq_tps.name)
            dose_tps_raw = tps_dist.dose
            res_tps_mm = tps_dist.resolution_mm
            st.success(f"✅ TPS: {dose_tps_raw.shape[1]}×{dose_tps_raw.shape[0]} px | {res_tps_mm:.3f} mm/px | Max: {dose_tps_raw.max():.2f} Gy")
        except Exception as e:
            st.error(f"Erro ao ler TPS: {e}")
            st.stop()

    # ============================================================
    # PARTE 3: REDIMENSIONAR TPS PARA TAMANHO DO FILME
    # ============================================================
    st.markdown("---")
    st.subheader("🔧 3. Redimensionar TPS para Tamanho do Filme")
    ...
    # ============================================================
    # PARTE 4: PREVIEW DOS DOIS MAPAS (MESMO TAMANHO)
    # ============================================================
    st.markdown("---")
    st.subheader("📊 4. Preview dos Mapas")

    fig_prev, ax_prev = plt.subplots(1, 2, figsize=(14, 6))
    vmax = max(float(np.max(dose_film)), float(np.max(dose_tps_resized)))

    im0 = ax_prev[0].imshow(dose_film, cmap='jet', vmin=0, vmax=vmax, origin='lower')
    ax_prev[0].set_title(f"Filme\n{dose_film.shape[1]}x{dose_film.shape[0]} | {res_film_mm:.3f} mm/px")
    ax_prev[0].axis('off')
    plt.colorbar(im0, ax=ax_prev[0], fraction=0.046)

    im1 = ax_prev[1].imshow(dose_tps_resized, cmap='jet', vmin=0, vmax=vmax, origin='lower')
    ax_prev[1].set_title(f"TPS Redimensionado\n{dose_tps_resized.shape[1]}x{dose_tps_resized.shape[0]} | {res_film_mm:.3f} mm/px")
    ax_prev[1].axis('off')
    ...
    # ============================================================
    # PARTE 5: REGISTRO COM FLIP AUTOMÁTICO
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 5. Registro Automático (com Flip)")

    st.info("O registro testa 4 orientações do filme e escolhe a melhor.")

    dose_film_aligned = dose_film
    best_flip = 'none'
    tx, ty = 0.0, 0.0
    reg_score = 0.0

    if st.button("🔧 Executar Registro Automático", type="primary"):
        with st.spinner("Testando 4 orientações..."):
            try:
                dose_film_aligned, best_flip, tx, ty, reg_score = register_with_flip(
                    dose_film, dose_tps_resized, res_film_mm
                )
                st.session_state['gg_film_aligned'] = dose_film_aligned
                st.session_state['gg_best_flip'] = best_flip
                st.session_state['gg_tx'] = tx
                st.session_state['gg_ty'] = ty
                st.session_state['gg_reg_score'] = reg_score
                st.success(f"✅ Melhor orientação: **{best_flip}** | Shift: X={tx:.1f}mm, Y={ty:.1f}mm | Score: {reg_score:.3f}")
            except Exception as e:
                st.error(f"Erro no registro: {e}")

    # Recuperar da sessão se disponível
    if 'gg_film_aligned' in st.session_state:
        dose_film_aligned = st.session_state['gg_film_aligned']
    ...
    with st.expander("⚙️ Ajuste Manual de Registro"):
        tx = st.slider("X (mm)", -200.0, 200.0, float(tx), 0.5, key="gg_sx")
        ty = st.slider("Y (mm)", -200.0, 200.0, float(ty), 0.5, key="gg_sy")
        flip_h = st.checkbox("Inverter Horizontalmente", value=(best_flip=='h' or best_flip=='hv'), key="gg_fh")
        flip_v = st.checkbox("Inverter Verticalmente", value=(best_flip=='v' or best_flip=='hv'), key="gg_fv")

    # Reaplicar flip manual
    dose_film_aligned = np.copy(dose_film)
    if flip_h:
        dose_film_aligned = np.fliplr(dose_film_aligned)
    if flip_v:
        dose_film_aligned = np.flipud(dose_film_aligned)

    # Aplicar shift ao TPS (usado no overlay e no gamma)
    dose_tps_shifted = apply_shift(dose_tps_resized, tx, ty, res_film_mm)

    # ============================================================
    # PARTE 6: OVERLAY
    # ============================================================
    st.markdown("---")
    st.subheader("📐 6. Sobreposição (Overlay)")

    fig_over, ax_over = plt.subplots(1, 2, figsize=(14, 6))

    # Sobreposição 1: TPS sobre filme
    ax_over[0].imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.5)
    ax_over[0].imshow(dose_tps_shifted, cmap='jet', origin='lower', alpha=0.5)
    ax_over[0].set_title("Sobreposição: Filme (cinza) + TPS (cor)")
    ax_over[0].axis('off')

    # Contornos
    levels = [0.5 * np.max(dose_film_aligned), 0.8 * np.max(dose_film_aligned)]
    ax_over[1].imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.4)
    ax_over[1].contour(dose_film_aligned, levels=levels, colors='yellow', linewidths=2, origin='lower')
    ax_over[1].contour(dose_tps_shifted, levels=levels, colors='cyan', linewidths=2, linestyles='--', origin='lower')
    ax_over[1].set_title("Contornos: Filme (amarelo) vs TPS (cyan)")
    ax_over[1].axis('off')
    ...
    # ============================================================
    # ============================================================
    # PARTE 7: ROI (REGIÃO DE INTERESSE)
    # ============================================================
    st.markdown("---")
    st.subheader("🎯 7. Região de Interesse (ROI)")

    roi_mode = st.radio("Modo de ROI", ["Automático (detecção do filme)", "Manual (coordenadas)"], index=0, key="gg_roi_mode")

    if roi_mode == "Automático (detecção do filme)":
        # ROI automática: detectar filme irradiado na imagem
        roi_mask_auto, bbox_auto = detect_film_roi(dose_film_aligned)

        # Mostrar ROI
        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.3)
        roi_display = np.where(roi_mask_auto, dose_tps_shifted, np.nan)
        im_roi = ax_roi.imshow(roi_display, cmap='jet', origin='lower', alpha=0.7)
        ...
        ax_roi.set_title(f"ROI Automática: {w}x{h} px | ({100*np.sum(roi_mask_auto)/roi_mask_auto.size:.1f}% da área)")
        ax_roi.axis('off')
        ...
        # ROI manual: coordenadas
        st.info("Defina as coordenadas do retângulo de ROI (em pixels):")

        h_img, w_img = dose_film_aligned.shape

        col_roi1, col_roi2 = st.columns(2)
        with col_roi1:
            roi_x = st.number_input("X inicial (px)", 0, w_img-1, int(w_img*0.1), key="gg_roi_x")
            roi_y = st.number_input("Y inicial (px)", 0, h_img-1, int(h_img*0.1), key="gg_roi_y")
        with col_roi2:
            roi_w = st.number_input("Largura (px)", 1, w_img-roi_x, int(w_img*0.8), key="gg_roi_w")
            roi_h = st.number_input("Altura (px)", 1, h_img-roi_y, int(h_img*0.8), key="gg_roi_h")

        # Criar máscara manual
        roi_mask_manual = np.zeros_like(dose_film_aligned, dtype=bool)
        roi_mask_manual[roi_y:roi_y+roi_h, roi_x:roi_x+roi_w] = True

        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.3)
        roi_display = np.where(roi_mask_manual, dose_tps_shifted, np.nan)
        im_roi = ax_roi.imshow(roi_display, cmap='jet', origin='lower', alpha=0.7)
        ...
        ax_roi.set_title(f"ROI Manual: {roi_w}x{roi_h} px | ({100*np.sum(roi_mask_manual)/roi_mask_manual.size:.1f}% da área)")
        ax_roi.axis('off')
        plt.colorbar(im_roi, ax=ax_roi, fraction=0.046)
        st.pyplot(fig_roi)

        roi_mask = roi_mask_manual

    st.info(f"📊 ROI cobre **{np.sum(roi_mask)} pixels** ({100*np.sum(roi_mask)/roi_mask.size:.1f}% da área)")


    # ============================================================
    # PARTE 8: PARÂMETROS GAMMA
    # ============================================================
    st.markdown("---")
    st.subheader("⚙️ 8. Parâmetros Gamma")

    c1, c2 = st.columns(2)
    with c1:
        dd = st.selectbox("Dose Difference (%)", [1, 2, 3, 5], index=2, key="gg_dd")
    with c2:
        dta = st.selectbox("DTA (mm)", [1, 2, 3, 5], index=2, key="gg_dta")

    global_norm = st.checkbox("Normalização Global", value=True, key="gg_gn")

    # ============================================================
    # PARTE 9: CALCULAR GAMMA
    # ============================================================
    st.markdown("---")
    if st.button("🚀 CALCULAR GAMMA", type="primary"):
        with st.spinner("Calculando gamma... (pode levar 1-2 min)"):
            try:
                gamma_res = gamma_analysis(
                    dose_film=dose_film_aligned,
                    dose_tps=dose_tps_shifted,
                    res_mm=res_film_mm,
                    roi_mask=roi_mask,
                    dose_percent=float(dd),
                    dta_mm=float(dta),
                    threshold_percent=10.0,  # threshold interno para ignorar dose muito baixa
    ...
    # PARTE 10: RESULTADOS
    # ============================================================
    if 'gg_result' in st.session_state:
        ...
        col_m1.metric("Passing Rate", f"{g['passing_rate']:.1f}%")
        col_m2.metric("γ Médio", f"{g['gamma_mean']:.3f}")
        col_m3.metric("γ Máximo", f"{g['gamma_max']:.3f}")

        # 4 mapas
        fig_r, ax_r = plt.subplots(2, 2, figsize=(14, 12))

        im0 = ax_r[0,0].imshow(dose_film_aligned, cmap='jet', origin='lower')
        ax_r[0,0].set_title(f"Filme ({best_flip}) - Max: {dose_film_aligned.max():.2f} Gy")
        ax_r[0,0].axis('off')
        plt.colorbar(im0, ax=ax_r[0,0], fraction=0.046)

        im1 = ax_r[0,1].imshow(dose_tps_shifted, cmap='jet', origin='lower')
        ax_r[0,1].set_title(f"TPS Alinhado - Max: {dose_tps_shifted.max():.2f} Gy")
        ax_r[0,1].axis('off')
        plt.colorbar(im1, ax=ax_r[0,1], fraction=0.046)

        gamma_map = g['gamma_map']
        gamma_disp = np.where(g['valid_mask'], gamma_map, np.nan)
        im2 = ax_r[1,0].imshow(gamma_disp, cmap='RdYlGn_r', vmin=0, vmax=2, origin='lower')
        ax_r[1,0].set_title(f"Gamma Index - {g['passing_rate']:.1f}% pass")
        ax_r[1,0].axis('off')
        plt.colorbar(im2, ax=ax_r[1,0], fraction=0.046)

        # Overlay final
        ax_r[1,1].imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.4)
        levels = [0.5 * np.max(dose_film_aligned), 0.8 * np.max(dose_film_aligned)]
        ax_r[1,1].contour(dose_film_aligned, levels=levels, colors='yellow', linewidths=2, origin='lower')
        ax_r[1,1].contour(dose_tps_shifted, levels=levels, colors='cyan', linewidths=2, linestyles='--', origin='lower')
        ax_r[1,1].set_title("Overlay: Filme (amarelo) + TPS (cyan)")
        ax_r[1,1].axis('off')
        ...
        ax_h.axvline(1.0, color='red', linestyle='--', linewidth=2, label='γ = 1.0')
        ...
        # Resumo
        st.subheader("📝 Resumo")
        st.write(f"- **Orientação do filme:** {best_flip}")
        st.write(f"- **Shift:** X={tx:.1f}mm, Y={ty:.1f}mm")
        st.write(f"- **Critério:** {dd}% / {dta}mm")
        st.write(f"- **Pass Rate:** {g['passing_rate']:.1f}%")
        st.write(f"- **γ médio:** {g['gamma_mean']:.3f}")
        st.write(f"- **γ máximo:** {g['gamma_max']:.3f}")
        st.write(f"- **Pixels avaliados:** {g['n_evaluated']} / {g['n_total']}")
