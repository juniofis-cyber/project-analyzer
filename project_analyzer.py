...
# ==================== ANÁLISE GAMMA ====================
    ...
    with st.expander("⚙️ Ajuste Manual de Registro"):
        tx = st.slider("X (mm)", -200.0, 200.0, st.session_state.get('gg_tx', 0.0), 0.5, key="gg_sx")
        ty = st.slider("Y (mm)", -200.0, 200.0, st.session_state.get('gg_ty', 0.0), 0.5, key="gg_sy")

    # Aplicar shift ao TPS (não ao filme)
    ...
    roi_mode = st.radio("Modo de ROI", ["Automático (detecção do filme)", "Manual (threshold de dose)"], index=0)
    ...
    if roi_mode == "Automático (detecção do filme)":
        ...
        ax_roi.set_title(f"ROI Automática: {w}×{h} px | ({100*np.sum(roi_mask_auto)/roi_mask_auto.size:.1f}% da área)")
        ax_roi.axis('off')
        ...
        # ROI manual: threshold de dose
        roi_threshold_pct = st.slider("Threshold da ROI (% da dose máxima do TPS)", 5, 95, 50, 5,
                                       help="A análise será feita apenas onde o TPS tem dose acima deste valor")
        roi_threshold = (roi_threshold_pct / 100.0) * np.max(dose_tps_shifted)
        roi_mask_manual = dose_tps_shifted > roi_threshold

        fig_roi, ax_roi = plt.subplots(figsize=(8, 6))
        ax_roi.imshow(dose_film_aligned, cmap='gray', origin='lower', alpha=0.3)
        roi_display = np.where(roi_mask_manual, dose_tps_shifted, np.nan)
        im_roi = ax_roi.imshow(roi_display, cmap='jet', origin='lower', alpha=0.7)
        ax_roi.set_title(f"ROI Manual: dose > {roi_threshold_pct}% ({roi_threshold:.2f} Gy)")
        ax_roi.axis('off')
        plt.colorbar(im_roi, ax=ax_roi, fraction=0.046)
        st.pyplot(fig_roi)
